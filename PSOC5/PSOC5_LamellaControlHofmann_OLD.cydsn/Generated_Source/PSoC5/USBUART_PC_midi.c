...
***************************************/
    ...
    static const uint8 CYCODE USBUART_PC_MIDI_SIZE[] = {
...
***************************************/
        ...
        * USBMIDI_MIDI_EP_Init() function.*/
        volatile uint16 USBUART_PC_midiInPointer;                            /* Input endpoint buffer pointer */
    #else
        volatile uint8 USBUART_PC_midiInPointer;                             /* Input endpoint buffer pointer */
    ...
    volatile uint8 USBUART_PC_midi_in_ep;
    ...
    uint8 USBUART_PC_midiInBuffer[USBUART_PC_MIDI_IN_BUFF_SIZE];       /* Input endpoint buffer */
    ...
    volatile uint8 USBUART_PC_midi_out_ep;                                   /* Output endpoint number */
    ...
    uint8 USBUART_PC_midiOutBuffer[USBUART_PC_MIDI_OUT_BUFF_SIZE];     /* Output endpoint buffer */
#endif /* (USBUART_PC_MIDI_OUT_BUFF_SIZE > 0) */

#if (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_ONE_EXT_INTRF)

    static USBUART_PC_MIDI_RX_STATUS USBUART_PC_MIDI1_Event;            /* MIDI RX status structure */
    static volatile uint8 USBUART_PC_MIDI1_TxRunStat;                         /* MIDI Output running status */
    ...
    volatile uint8 USBUART_PC_MIDI1_InqFlags;                                 /* Device inquiry flag */

    #if (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_TWO_EXT_INTRF)
        static USBUART_PC_MIDI_RX_STATUS USBUART_PC_MIDI2_Event;        /* MIDI RX status structure */
        static volatile uint8 USBUART_PC_MIDI2_TxRunStat;                     /* MIDI Output running status */
        /** See description of \ref USBUART_PC_MIDI1_InqFlags*/
        volatile uint8 USBUART_PC_MIDI2_InqFlags;                             /* Device inquiry flag */
...
#if (USBUART_PC_ENABLE_MIDI_API != 0u)
/*******************************************************************************
* Function Name: USBUART_PC_MIDI_Init
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBUART_PC_midiInBuffer: This buffer is used for saving and combining
*    the received data from UART(s) and(or) generated internally by
*    PutUsbMidiIn() function messages. USBUART_PC_MIDI_IN_EP_Service()
*    function transfers the data from this buffer to the PC.
*
*  \ref USBUART_PC_midiOutBuffer: This buffer is used by the
*    USBUART_PC_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref USBUART_PC_callbackLocalMidiEvent function.
*
*  \ref USBUART_PC_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref USBUART_PC_midi_in_ep: Used as an IN endpoint number.
*
*   \ref USBUART_PC_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void USBUART_PC_MIDI_Init(void) 
{
   ...
   USBUART_PC_midiInPointer = 0u;
        ...
        USBUART_PC_LoadInEP(USBUART_PC_midi_in_ep, USBUART_PC_midiInBuffer,
                                                               USBUART_PC_MIDI_IN_BUFF_SIZE);
    #endif  /* (USBUART_PC_MIDI_IN_BUFF_SIZE > 0) */

    #if (USBUART_PC_MIDI_OUT_BUFF_SIZE > 0)
        /* Provide buffer for OUT endpoint. */
        (void)USBUART_PC_ReadOutEP(USBUART_PC_midi_out_ep, USBUART_PC_midiOutBuffer,
                                                                       USBUART_PC_MIDI_OUT_BUFF_SIZE);
    #endif /* (USBUART_PC_MIDI_OUT_BUFF_SIZE > 0) */
#endif /* (USBUART_PC_EP_MANAGEMENT_DMA_AUTO) */

#if (USBUART_PC_MIDI_OUT_BUFF_SIZE > 0)
    USBUART_PC_EnableOutEP(USBUART_PC_midi_out_ep);
#endif /* (USBUART_PC_MIDI_OUT_BUFF_SIZE > 0) */

    /* Initialize the MIDI port(s) */
#if (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_ONE_EXT_INTRF)
    USBUART_PC_MIDI_InitInterface();
#endif /* (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_ONE_EXT_INTRF) */
}
...
#if (USBUART_PC_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBUART_PC_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref USBUART_PC_midiOutBuffer: Used as temporary buffer between USB
    ...
    *******************************************************************************/
    ...
    {
    #if (USBUART_PC_MIDI_OUT_BUFF_SIZE >= 256)
        uint16 outLength;
        uint16 outPointer;
    #else
        uint8 outLength;
        uint8 outPointer;
    #endif /* (USBUART_PC_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (USBUART_PC_OUT_BUFFER_FULL == USBUART_PC_GetEPState(USBUART_PC_midi_out_ep))
        {
        #if (USBUART_PC_MIDI_OUT_BUFF_SIZE >= 256)
            outLength = USBUART_PC_GetEPCount(USBUART_PC_midi_out_ep);
        #else
            outLength = (uint8)USBUART_PC_GetEPCount(USBUART_PC_midi_out_ep);
        #endif /* (USBUART_PC_MIDI_OUT_BUFF_SIZE >= 256) */

        #if (!USBUART_PC_EP_MANAGEMENT_DMA_AUTO)
            #if (USBUART_PC_MIDI_OUT_BUFF_SIZE >= 256)
                outLength = USBUART_PC_ReadOutEP(USBUART_PC_midi_out_ep,
                                                       USBUART_PC_midiOutBuffer, outLength);
            #else
                outLength = (uint8)USBUART_PC_ReadOutEP(USBUART_PC_midi_out_ep,
                                                              USBUART_PC_midiOutBuffer, (uint16) outLength);
            #endif /* (USBUART_PC_MIDI_OUT_BUFF_SIZE >= 256) */

            #if (USBUART_PC_EP_MANAGEMENT_DMA_MANUAL)
                /* Wait until DMA complete transferring data from OUT endpoint buffer. */
                while (USBUART_PC_OUT_BUFFER_FULL == USBUART_PC_GetEPState(USBUART_PC_midi_out_ep))
                {
                }
            ...
            if (outLength >= USBUART_PC_EVENT_LENGTH)
            {
                ...
                #if (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_ONE_EXT_INTRF)
                    if ((USBUART_PC_midiOutBuffer[outPointer] & USBUART_PC_CABLE_MASK) ==
                         USBUART_PC_MIDI_CABLE_00)
                    {
                        USBUART_PC_MIDI1_ProcessUsbOut(&USBUART_PC_midiOutBuffer[outPointer]);
                    }
                    else if ((USBUART_PC_midiOutBuffer[outPointer] & USBUART_PC_CABLE_MASK) ==
                             USBUART_PC_MIDI_CABLE_01)
                    {
                    #if (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_TWO_EXT_INTRF)
                         USBUART_PC_MIDI2_ProcessUsbOut(&USBUART_PC_midiOutBuffer[outPointer]);
                    #endif /*  USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_TWO_EXT_INTRF */
                    }
                    else
                    {
                        ...
                        #ifdef USBUART_PC_MIDI_OUT_EP_SERVICE_CALLBACK
                            USBUART_PC_MIDI_OUT_EP_Service_Callback();
                        #endif /* USBUART_PC_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    USBUART_PC_callbackLocalMidiEvent(USBUART_PC_midiOutBuffer[outPointer] & USBUART_PC_CABLE_MASK,
                                                            &USBUART_PC_midiOutBuffer[outPointer + USBUART_PC_EVENT_BYTE1]);
                    outPointer += USBUART_PC_EVENT_LENGTH;
                }
            }
...
#if (USBUART_PC_MIDI_IN_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBUART_PC_MIDI_IN_EP_Service
    ****************************************************************************//**
    *
    ...
    *  USBUART_PC_MIDI_IN_Service() and from the
    *  USBUART_PC_PutUsbMidiIn() function.
    *
    * \globalvars
    *  USBUART_PC_midi_in_ep: Used as an IN endpoint number.
    *  USBUART_PC_midiInBuffer: Function loads the data from this buffer to
    *    the USB IN endpoint.
    *   USBUART_PC_midiInPointer: Cleared to zero when data are sent.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void USBUART_PC_MIDI_IN_EP_Service(void) 
    {
        ...
        if (USBUART_PC_midiInPointer != 0u)
        {
            if(USBUART_PC_GetEPState(USBUART_PC_midi_in_ep) == USBUART_PC_EVENT_PENDING)
            {
            #if (USBUART_PC_EP_MANAGEMENT_DMA_AUTO)
                USBUART_PC_LoadInEP(USBUART_PC_midi_in_ep, NULL, (uint16)USBUART_PC_midiInPointer);
            #else
                USBUART_PC_LoadInEP(USBUART_PC_midi_in_ep, USBUART_PC_midiInBuffer,
                                                              (uint16) USBUART_PC_midiInPointer);
            #endif /* (USBUART_PC_EP_MANAGEMENT_DMA_AUTO) */
    ...
    *******************************************************************************/
    void USBUART_PC_MIDI_IN_Service(void) 
    {
        ...
        */
        ...
        if (0u == USBUART_PC_midiInPointer)
        {
            do
            {
                if (USBUART_PC_midiInPointer <= (USBUART_PC_MIDI_IN_BUFF_SIZE - USBUART_PC_EVENT_LENGTH))
                {
                    /* Check MIDI1 input port for a complete event */
                    m1 = USBUART_PC_MIDI1_GetEvent();
                    if (m1 != 0u)
                    {
                        USBUART_PC_PrepareInBuffer(m1, (uint8 *)&USBUART_PC_MIDI1_Event.msgBuff[0],
                                                                       USBUART_PC_MIDI1_Event.size, USBUART_PC_MIDI_CABLE_00);
                    }
                }
                ...
                if (USBUART_PC_midiInPointer <= (USBUART_PC_MIDI_IN_BUFF_SIZE - USBUART_PC_EVENT_LENGTH))
                {
                    ...
                    m2 = USBUART_PC_MIDI2_GetEvent();
                    if (m2 != 0u)
                    {
                        USBUART_PC_PrepareInBuffer(m2, (uint8 *)&USBUART_PC_MIDI2_Event.msgBuff[0],
                                                                       USBUART_PC_MIDI2_Event.size, USBUART_PC_MIDI_CABLE_01);
                    }
                }
            #endif /*  USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_TWO_EXT_INTRF */
            }
            while((USBUART_PC_midiInPointer <= (USBUART_PC_MIDI_IN_BUFF_SIZE - USBUART_PC_EVENT_LENGTH)) &&
                  ((m1 != 0u) || (m2 != 0u)));
        }
    #endif /* (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_ONE_EXT_INTRF) */

        /* Service the USB MIDI input endpoint */
        USBUART_PC_MIDI_IN_EP_Service();
    }


    /***************************************************************************
    * Function Name: USBUART_PC_PutUsbMidiIn
    ************************************************************************//**
    *
    ...
    *  The USBUART_PC_MIDI_IN_Service() function should also be called to
    ...
    *   ----------------------|-----------------------------------------
    *  USBUART_PC_TRUE  | Host is not ready to receive this message
    *  USBUART_PC_FALSE | Success transfer
    *
    * \globalvars
    *
    *  \ref USBUART_PC_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref USBUART_PC_midiInPointer: Checked this variable to see if
    ...
    ***************************************************************************/
    ...
    {
        uint8 retError = USBUART_PC_FALSE;
        ...
        if (USBUART_PC_midiInPointer >
                    (USBUART_PC_EP[USBUART_PC_midi_in_ep].bufferSize - USBUART_PC_EVENT_LENGTH))
        {
            USBUART_PC_MIDI_IN_EP_Service();
        }
        ...
        if (USBUART_PC_midiInPointer <=
                    (USBUART_PC_EP[USBUART_PC_midi_in_ep].bufferSize - USBUART_PC_EVENT_LENGTH))
        {
            if((ic < USBUART_PC_EVENT_LENGTH) || (ic >= USBUART_PC_MIDI_STATUS_MASK))
            {
                USBUART_PC_PrepareInBuffer(ic, midiMsg, ic, cable);
            }
            else /* Only SysEx message is greater than 4 bytes */
            {
                ...
                do
                {
                    ...
                    USBUART_PC_PrepareInBuffer(USBUART_PC_MIDI_SYSEX, &midiMsg[idx],
                                                     USBUART_PC_EVENT_BYTE3, cable);

                    /* Move to next 3 bytes of message */
                    ic  -= USBUART_PC_EVENT_BYTE3;
                    idx += USBUART_PC_EVENT_BYTE3;
                        ...
                        (USBUART_PC_EP[USBUART_PC_midi_in_ep].bufferSize - USBUART_PC_EVENT_LENGTH))
                    {
                           ...
                           (USBUART_PC_EP[USBUART_PC_midi_in_ep].bufferSize - USBUART_PC_EVENT_LENGTH))
                        {
                            ...
                            retError = USBUART_PC_TRUE;
                            break;
                        }
                    }
                }
                while (ic >= USBUART_PC_EVENT_BYTE3);

                if (retError == USBUART_PC_FALSE)
                {
                    /* Handle end of message: valid size of messages is 0, 1 and 2 */
                    USBUART_PC_PrepareInBuffer(USBUART_PC_MIDI_EOSEX, &midiMsg[idx], ic, cable);
                }
            }
        }
        else
    ...
    /*******************************************************************************
    * Function Name: USBUART_PC_PrepareInBuffer
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBUART_PC_midiInBuffer: This buffer is used for saving and combine the
    *    received from UART(s) and(or) generated internally by
    *    USBUART_PC_PutUsbMidiIn() function messages.
    *  USBUART_PC_midiInPointer: Used as an index for midiInBuffer to
    *     write data.
    *
    * \reentrant
    *  No
    ...
    *******************************************************************************/
                                                                 ...
                                                                 
    {
        ...
        if (ic >= (USBUART_PC_MIDI_STATUS_MASK | USBUART_PC_MIDI_SINGLE_BYTE_MASK))
        {
            USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_SINGLE_BYTE | cable;
            USBUART_PC_midiInPointer++;
            USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = ic;
            USBUART_PC_midiInPointer++;
            USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = 0u;
            USBUART_PC_midiInPointer++;
            USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = 0u;
            USBUART_PC_midiInPointer++;
        }
        else if((ic < USBUART_PC_EVENT_LENGTH) || (ic == USBUART_PC_MIDI_SYSEX))
        {
            if(ic == USBUART_PC_MIDI_SYSEX)
            {
                USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_SYSEX | cable;
                USBUART_PC_midiInPointer++;
            }
            else if (srcBuffZero < USBUART_PC_MIDI_SYSEX)
            {
                USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = (srcBuffZero >> 4u) | cable;
                USBUART_PC_midiInPointer++;
            }
            else if (srcBuffZero == USBUART_PC_MIDI_TUNEREQ)
            {
                USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_1BYTE_COMMON | cable;
                USBUART_PC_midiInPointer++;
            }
            else if ((srcBuffZero == USBUART_PC_MIDI_QFM) || (srcBuffZero == USBUART_PC_MIDI_SONGSEL))
            {
                USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_2BYTE_COMMON | cable;
                USBUART_PC_midiInPointer++;
            }
            else if (srcBuffZero == USBUART_PC_MIDI_SPP)
            {
                USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_3BYTE_COMMON | cable;
                USBUART_PC_midiInPointer++;
            }
            else
            {
            ...
            USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = srcBuffZero;
            USBUART_PC_midiInPointer++;
            USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = srcBuffOne;
            USBUART_PC_midiInPointer++;
            USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = srcBuff[2u];
            USBUART_PC_midiInPointer++;
        }
        else if (ic == USBUART_PC_MIDI_EOSEX)
        {
            switch (eventLen)
            {
                case 0u:
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_SYSEX_ENDS_WITH1 | cable;
                    USBUART_PC_midiInPointer++;
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_MIDI_EOSEX;
                    USBUART_PC_midiInPointer++;
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = 0u;
                    USBUART_PC_midiInPointer++;
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = 0u;
                    ...
                    break;
                case 1u:
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_SYSEX_ENDS_WITH2 | cable;
                    USBUART_PC_midiInPointer++;
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = srcBuffZero;
                    USBUART_PC_midiInPointer++;
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_MIDI_EOSEX;
                    ...
                    break;
                case 2u:
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = USBUART_PC_SYSEX_ENDS_WITH3 | cable;
                    USBUART_PC_midiInPointer++;
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = srcBuffZero;
                    USBUART_PC_midiInPointer++;
                    USBUART_PC_midiInBuffer[USBUART_PC_midiInPointer] = srcBuffOne;
...
*/
#if (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBUART_PC_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *   USBUART_PC_MIDI_Event: initialized to zero.
    *   USBUART_PC_MIDI_TxRunStat: initialized to zero.
    *
    *******************************************************************************/
    void USBUART_PC_MIDI_InitInterface(void) 
    {
        USBUART_PC_MIDI1_Event.length  = 0u;
        USBUART_PC_MIDI1_Event.count   = 0u;
        USBUART_PC_MIDI1_Event.size    = 0u;
        USBUART_PC_MIDI1_Event.runstat = 0u;
        USBUART_PC_MIDI1_TxRunStat     = 0u;
        USBUART_PC_MIDI1_InqFlags      = 0u;

        /* Start UART block */
        MIDI1_UART_Start();

        /* Change the priority of the UART TX and RX interrupt */
        CyIntSetPriority(MIDI1_UART_TX_VECT_NUM, USBUART_PC_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI1_UART_RX_VECT_NUM, USBUART_PC_CUSTOM_UART_RX_PRIOR_NUM);

    #if (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_TWO_EXT_INTRF)
        USBUART_PC_MIDI2_Event.length  = 0u;
        USBUART_PC_MIDI2_Event.count   = 0u;
        USBUART_PC_MIDI2_Event.size    = 0u;
        USBUART_PC_MIDI2_Event.runstat = 0u;
        USBUART_PC_MIDI2_TxRunStat     = 0u;
        USBUART_PC_MIDI2_InqFlags      = 0u;

        /* Start second UART block */
        MIDI2_UART_Start();

        /* Change the priority of the UART TX interrupt */
        CyIntSetPriority(MIDI2_UART_TX_VECT_NUM, USBUART_PC_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI2_UART_RX_VECT_NUM, USBUART_PC_CUSTOM_UART_RX_PRIOR_NUM);
    #endif /* (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_TWO_EXT_INTRF) */
    ...
    #ifdef USBUART_PC_MIDI_INIT_CALLBACK
        USBUART_PC_MIDI_Init_Callback();
    #endif /* (USBUART_PC_MIDI_INIT_CALLBACK) */
    }


    /*******************************************************************************
    * Function Name: USBUART_PC_ProcessMidiIn
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & USBUART_PC_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            if ((mData & USBUART_PC_MIDI_STATUS_MASK) == USBUART_PC_MIDI_STATUS_MASK)
            {
                if ((mData & USBUART_PC_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(single byte) */
                {
                    midiReturn = mData;
                }
                else                              /* System Common Messages */
                {
                    switch (mData)
                    {
                        case USBUART_PC_MIDI_SYSEX:
                            rxStat->msgBuff[0u] = USBUART_PC_MIDI_SYSEX;
                            rxStat->runstat = USBUART_PC_MIDI_SYSEX;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBUART_PC_MIDI_EOSEX:
                            rxStat->runstat = 0u;
                            rxStat->size = rxStat->count;
                            rxStat->count = 0u;
                            midiReturn = USBUART_PC_MIDI_EOSEX;
                            break;
                        case USBUART_PC_MIDI_SPP:
                            rxStat->msgBuff[0u] = USBUART_PC_MIDI_SPP;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBUART_PC_MIDI_SONGSEL:
                            rxStat->msgBuff[0u] = USBUART_PC_MIDI_SONGSEL;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBUART_PC_MIDI_QFM:
                            rxStat->msgBuff[0u] = USBUART_PC_MIDI_QFM;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBUART_PC_MIDI_TUNEREQ:
                            rxStat->msgBuff[0u] = USBUART_PC_MIDI_TUNEREQ;
                            rxStat->runstat = 0u;
                            rxStat->size = 1u;
                            rxStat->count = 0u;
            ...
            else /* Channel Messages */
            {
                ...
                rxStat->count = 1u;
                switch (mData & USBUART_PC_MIDI_STATUS_MASK)
                {
                    case USBUART_PC_MIDI_NOTE_OFF:
                    case USBUART_PC_MIDI_NOTE_ON:
                    case USBUART_PC_MIDI_POLY_KEY_PRESSURE:
                    case USBUART_PC_MIDI_CONTROL_CHANGE:
                    case USBUART_PC_MIDI_PITCH_BEND_CHANGE:
                        rxStat->length = 3u;
                        break;
                    case USBUART_PC_MIDI_PROGRAM_CHANGE:
                    case USBUART_PC_MIDI_CHANNEL_PRESSURE:
                        rxStat->length = 2u;
                        break;
                    default:
                        rxStat->runstat = 0u;
        ...
        {
            if (rxStat->runstat == USBUART_PC_MIDI_SYSEX)
            {
                ...
                if (rxStat->count >= rxStat->length)
                {
                    rxStat->size = rxStat->count;
                    rxStat->count = 0u;
                    midiReturn = USBUART_PC_MIDI_SYSEX;
                }
            }
            ...
            else if (rxStat->runstat != 0u)
            {
                ...
                rxStat->count = 2u;
                switch (rxStat->runstat & USBUART_PC_MIDI_STATUS_MASK)
                {
                    case USBUART_PC_MIDI_NOTE_OFF:
                    case USBUART_PC_MIDI_NOTE_ON:
                    case USBUART_PC_MIDI_POLY_KEY_PRESSURE:
    ...
    /*******************************************************************************
    * Function Name: USBUART_PC_MIDI1_GetEvent
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBUART_PC_MIDI1_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /*  ((MIDI1_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBUART_PC_ProcessMidiIn(rxData,
                                                    (USBUART_PC_MIDI_RX_STATUS *)&USBUART_PC_MIDI1_Event);
    ...
    /*******************************************************************************
    * Function Name: USBUART_PC_MIDI1_ProcessUsbOut
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBUART_PC_MIDI1_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBUART_PC_MIDI1_InqFlags: The following flags are set when SysEx
    *    message comes.
    *    USBUART_PC_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *    USBUART_PC_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    ...
    *******************************************************************************/
    ...
    #ifdef USBUART_PC_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBUART_PC_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (USBUART_PC_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */

        cmd = epBuf[USBUART_PC_EVENT_BYTE0] & USBUART_PC_CIN_MASK;

        if ((cmd != USBUART_PC_RESERVED0) && (cmd != USBUART_PC_RESERVED1))
        {
            len = USBUART_PC_MIDI_SIZE[cmd];
            i = USBUART_PC_EVENT_BYTE1;
            /* Universal System Exclusive message parsing */
            if (cmd == USBUART_PC_SYSEX)
            {
                if ((epBuf[USBUART_PC_EVENT_BYTE1] == USBUART_PC_MIDI_SYSEX) &&
                    (epBuf[USBUART_PC_EVENT_BYTE2] == USBUART_PC_MIDI_SYSEX_NON_REAL_TIME))
                {
                    /* Non-Real Time SySEx starts */
                    USBUART_PC_MIDI1_InqFlags |= USBUART_PC_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBUART_PC_MIDI1_InqFlags &= (uint8)~USBUART_PC_INQ_SYSEX_FLAG;
                }
            }
            else if (cmd == USBUART_PC_SYSEX_ENDS_WITH1)
            {
                USBUART_PC_MIDI1_InqFlags &= (uint8)~USBUART_PC_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBUART_PC_SYSEX_ENDS_WITH2)
            {
                USBUART_PC_MIDI1_InqFlags &= (uint8)~USBUART_PC_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBUART_PC_SYSEX_ENDS_WITH3)
            {
                /* Identify Request support */
                if ((USBUART_PC_MIDI1_InqFlags & USBUART_PC_INQ_SYSEX_FLAG) != 0u)
                {
                    USBUART_PC_MIDI1_InqFlags &= (uint8)~USBUART_PC_INQ_SYSEX_FLAG;
                    if ((epBuf[USBUART_PC_EVENT_BYTE1] == USBUART_PC_MIDI_SYSEX_GEN_INFORMATION) &&
                        (epBuf[USBUART_PC_EVENT_BYTE2] == USBUART_PC_MIDI_SYSEX_IDENTITY_REQ))
                    {
                        /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBUART_PC_MIDI1_InqFlags |= USBUART_PC_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            ...
            if ((cmd >= USBUART_PC_NOTE_OFF) && (cmd <= USBUART_PC_PITCH_BEND_CHANGE))
            {
                if (USBUART_PC_MIDI1_TxRunStat == epBuf[USBUART_PC_EVENT_BYTE1])
                {
                ...
                else
                {
                    ...
                    USBUART_PC_MIDI1_TxRunStat = epBuf[USBUART_PC_EVENT_BYTE1];
                }
            }
            else
            {
    ...
    #ifdef USBUART_PC_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        USBUART_PC_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (USBUART_PC_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }


#if (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_TWO_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBUART_PC_MIDI2_GetEvent
    ****************************************************************************//**
    *
    *  Checks for incoming MIDI data, calls the MIDI event builder if so.
    *  Returns either empty or with a complete event.
    ...
    *
    * \globalvars
    *  USBUART_PC_MIDI2_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /* ((MIDI2_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBUART_PC_ProcessMidiIn(rxData,
                                                    (USBUART_PC_MIDI_RX_STATUS *)&USBUART_PC_MIDI2_Event);
    ...
    /*******************************************************************************
    * Function Name: USBUART_PC_MIDI2_ProcessUsbOut
    ****************************************************************************//**
    *
    *  Process a USB MIDI output event.
    *  Puts data into the MIDI TX output buffer.
    ...
    *
    * \globalvars
    *  USBUART_PC_MIDI2_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBUART_PC_MIDI2_InqFlags: The following flags are set when SysEx
    *    message comes.
    *  USBUART_PC_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *  USBUART_PC_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *    This bit should be cleared by user when Identity Reply message generated.
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
    ...
    #ifdef USBUART_PC_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBUART_PC_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (USBUART_PC_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
        ...
        if ((cmd != USBUART_PC_RESERVED0) && (cmd != USBUART_PC_RESERVED1))
        {
            ...
            if(cmd == USBUART_PC_SYSEX)
            {
                ...
                {
                    /* SySEx starts */
                    USBUART_PC_MIDI2_InqFlags |= USBUART_PC_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBUART_PC_MIDI2_InqFlags &= (uint8)~USBUART_PC_INQ_SYSEX_FLAG;
                }
            }
            else if(cmd == USBUART_PC_SYSEX_ENDS_WITH1)
            {
            ...
            {
                /* Identify Request support */
                if ((USBUART_PC_MIDI2_InqFlags & USBUART_PC_INQ_SYSEX_FLAG) != 0u)
                {
                    USBUART_PC_MIDI2_InqFlags &= (uint8)~USBUART_PC_INQ_SYSEX_FLAG;
                    ...
                    {   /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBUART_PC_MIDI2_InqFlags |= USBUART_PC_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            else /* Do nothing for other command */
            ...
            if ((cmd >= USBUART_PC_NOTE_OFF) && ( cmd <= USBUART_PC_PITCH_BEND_CHANGE))
            {
                if (USBUART_PC_MIDI2_TxRunStat == epBuf[USBUART_PC_EVENT_BYTE1])
                {   /* Skip the repeated Status byte */
                    i++;
                }
                else
                {   /* Save Status byte for next event */
                    USBUART_PC_MIDI2_TxRunStat = epBuf[USBUART_PC_EVENT_BYTE1];
                }
            }
            else
            {   /* Clear Running Status */
    ...
    #ifdef USBUART_PC_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        USBUART_PC_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (USBUART_PC_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_TWO_EXT_INTRF) */
#endif /* (USBUART_PC_MIDI_EXT_MODE >= USBUART_PC_ONE_EXT_INTRF) */
...
#endif  /* defined(USBUART_PC_ENABLE_MIDI_STREAMING) */
