 ...
 * USBUART_PC_GetDTERate(), USBUART_PC_GetCharFormat(),
 * USBUART_PC_GetParityType(), and USBUART_PC_GetDataBits() APIs.
...
volatile uint8 USBUART_PC_linesCoding[USBUART_PC_MAX_MULTI_COM_NUM][USBUART_PC_LINE_CODING_SIZE] =
{
    /*COM Port 1*/
    {
...
/**Used as a flag for the USBUART_PC_IsLineChanged() API, to inform it that the
 ...
 * support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBUART_PC_linesChanged[USBUART_PC_MAX_MULTI_COM_NUM];
 ...
 * USBUART_PC_GetLineControl() API. It is an array of 2 elements for COM 
 * port 1 and COM port 2 for MultiCOM port support. In case of 1 COM port, data 
 * is in 0 element.*/
volatile uint16 USBUART_PC_linesControlBitmap[USBUART_PC_MAX_MULTI_COM_NUM];
/** Contains the 16-bit serial state value that was sent using the 
 * \ref USBUART_PC_SendSerialState() API. . It is an array of 2 elements 
 * for COM port 1 and COM port 2 for MultiCOM port support. In case of 1 COM 
 * port, data is in 0 element.*/
volatile uint16 USBUART_PC_serialStateBitmap[USBUART_PC_MAX_MULTI_COM_NUM];
...
volatile uint8  USBUART_PC_cdcDataInEp[USBUART_PC_MAX_MULTI_COM_NUM];
 ...
 * port 2 for MultiCOM port support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBUART_PC_cdcDataOutEp[USBUART_PC_MAX_MULTI_COM_NUM];
...
volatile uint8  USBUART_PC_cdcCommInInterruptEp[USBUART_PC_MAX_MULTI_COM_NUM];

/*PRIVATE*/

#define USBUART_PC_CDC_IN_EP      (0u)
#define USBUART_PC_CDC_OUT_EP     (1u)
#define USBUART_PC_CDC_NOTE_EP    (2u)

#define USBUART_PC_CDC_EP_MASK    (0x01u)

#define USBUART_PC_GET_EP_COM_NUM(cdcComNums, epType) (((cdcComNums) >> (epType)) & USBUART_PC_CDC_EP_MASK)
...
***************************************/
    ...
    static uint16 USBUART_PC_StrLen(const char8 string[]) ;
    static t_USBUART_PC_cdc_notification USBUART_PC_serialStateNotification =
    {

        USBUART_PC_SERIAL_STATE_REQUEST_TYPE, /* bRequestType    */
        USBUART_PC_SERIAL_STATE,              /* bNotification   */
        0u,                                         /* wValue          */
        0u,                                         /* wValueMSB       */
        0u,                                         /* wIndex          */
        0u,                                         /* wIndexMSB       */
        USBUART_PC_SERIAL_STATE_LENGTH,       /* wLength         */
        0u,                                         /* wLengthMSB      */
        0u,                                         /* wSerialState    */
        0u,                                         /* wSerialStateMSB */
    };
    static uint8 USBUART_PC_activeCom = 0u;
#endif /* (USBUART_PC_ENABLE_CDC_CLASS_API != 0u) */
...
/*******************************************************************************
* Function Name: USBUART_PC_DispatchCDCClassRqst
****************************************************************************//**
*
...
*
* \globalvars
*   USBUART_PC_linesCoding: Contains the current line coding structure.
...
*   USBUART_PC_linesControlBitmap: Contains the current control signal
*     bitmap. It is set by the Host using SET_CONTROL_LINE request and returned
*     to the user code by the USBFS_GetLineControl() API.
*   USBUART_PC_linesChanged: This variable is used as a flag for the
...
uint8 USBUART_PC_DispatchCDCClassRqst(void) 
{
    ...
    comPort = USBUART_PC_GetInterfaceComPort((uint8)USBUART_PC_wIndexLoReg);
    ...
    if (0u != (USBUART_PC_bmRequestTypeReg & USBUART_PC_RQST_DIR_D2H))
    {
        ...
        {
            case USBUART_PC_CDC_GET_LINE_CODING:
                USBUART_PC_currentTD.count = USBUART_PC_LINE_CODING_SIZE;
                USBUART_PC_currentTD.pData = USBUART_PC_linesCoding[comPort];
            ...
            #ifdef USBUART_PC_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK
                requestHandled = USBUART_PC_DispatchCDCClass_CDC_READ_REQUESTS_Callback();
            #endif /* (USBUART_PC_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK) */
                break;
        }
    }
    else
    {
        ...
        switch (USBUART_PC_bRequestReg)
        {
            case USBUART_PC_CDC_SET_LINE_CODING:
                USBUART_PC_currentTD.count  = USBUART_PC_LINE_CODING_SIZE;
                USBUART_PC_currentTD.pData  = USBUART_PC_linesCoding[comPort];
                USBUART_PC_linesChanged[comPort] |= USBUART_PC_LINE_CODING_CHANGED;

                requestHandled = USBUART_PC_InitControlWrite();
                break;

            case USBUART_PC_CDC_SET_CONTROL_LINE_STATE:
                USBUART_PC_linesControlBitmap[comPort] = (uint8) USBUART_PC_wValueLoReg;
                USBUART_PC_linesChanged[comPort]      |= USBUART_PC_LINE_CONTROL_CHANGED;
            ...
            default:
                /* Do not handle this request unless callback is defined. */
            #ifdef USBUART_PC_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK
                requestHandled = USBUART_PC_DispatchCDCClass_CDC_WRITE_REQUESTS_Callback();
            #endif /* (USBUART_PC_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK) */
                break;
        }
    }
...
/***************************************************************************
* Function Name: USBUART_PC_GetInterfaceComPort
************************************************************************//**
*   \internal
...
uint8 USBUART_PC_GetInterfaceComPort(uint8 interface) 
{
    ...
    while (comPort < USBUART_PC_MAX_MULTI_COM_NUM)
    {
        notEp = USBUART_PC_cdcCommInInterruptEp[comPort];

        if (USBUART_PC_EP[notEp].interface == interface)
        {
            res = comPort;
            comPort = USBUART_PC_MAX_MULTI_COM_NUM;
        }
...
#if (USBUART_PC_ENABLE_CDC_CLASS_API != 0u)
/***************************************************************************
* Function Name: USBUART_PC_CDC_Init
************************************************************************//**
*
...
*   USBUART_PC_SUCCESS   CDC interface was initialized correctly
*   USBUART_PC_FAILURE   CDC interface was not initialized
*
* \globalvars
*   USBUART_PC_linesChanged: Initialized to zero.
*   USBUART_PC_cdcDataOutEp: Used as an OUT endpoint number.
*
* \reentrant
*  No.
*
...
uint8 USBUART_PC_CDC_Init(void) 
{
    uint8 comPort;
    uint8 outEp;
    uint8 ret = USBUART_PC_SUCCESS;

    USBUART_PC_activeCom = 0u;
    USBUART_PC_linesChanged[USBUART_PC_COM_PORT1] = 0u;
    USBUART_PC_linesChanged[USBUART_PC_COM_PORT2] = 0u;

    for(comPort = 0u; comPort<USBUART_PC_MAX_MULTI_COM_NUM; comPort++)
    {
        outEp = USBUART_PC_cdcDataOutEp[comPort];
        if((0u != outEp) && (USBUART_PC_MAX_EP > outEp))
        {
            USBUART_PC_EnableOutEP(outEp);
        }

    }

    /* COM Port 1 should be correct to proceed. */
    if ((0u == USBUART_PC_cdcDataInEp[USBUART_PC_COM_PORT1]) \
            || (0u == USBUART_PC_cdcDataOutEp[USBUART_PC_COM_PORT1]) \
            || (0u ==  USBUART_PC_cdcCommInInterruptEp[USBUART_PC_COM_PORT1])
            || (USBUART_PC_cdcDataInEp[USBUART_PC_COM_PORT1] >= USBUART_PC_MAX_EP)
            || (USBUART_PC_cdcDataOutEp[USBUART_PC_COM_PORT1] >= USBUART_PC_MAX_EP)
            || (USBUART_PC_cdcCommInInterruptEp[USBUART_PC_COM_PORT1] >= USBUART_PC_MAX_EP))
    {
        ret = USBUART_PC_FAILURE;
    }
...
/*******************************************************************************
* Function Name: USBUART_PC_PutData
****************************************************************************//**
*
*  This function sends a specified number of bytes from the location specified
*  by a pointer to the PC. The USBUART_PC_CDCIsReady() function should be
...
*  identified. To send zero-length packet, use USBUART_PC_PutData() API
...
* \globalvars
*
*   USBUART_PC_cdcDataInEp: CDC IN endpoint number used for sending
*     data.
*
* \reentrant
*  No.
...
{
    uint8 epNumber = USBUART_PC_cdcDataInEp[USBUART_PC_activeCom];

    /* Limit length to maximum packet size for endpoint. */
    if (length > USBUART_PC_EP[epNumber].bufferSize)
    {
        /* Caution: Data will be lost if length is greater than Max Packet Size. */
        length = USBUART_PC_EP[epNumber].bufferSize;

        /* Halt CPU in debug mode */
        CYASSERT(0u != 0u);
    }

    USBUART_PC_LoadInEP(epNumber, pData, length);
}


/*******************************************************************************
* Function Name: USBUART_PC_StrLen
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USBUART_PC_PutString
************************************************************************//**
*
...
*  The USBUART_PC_CDCIsReady() function should be called before
*  sending data with a new call to USBUART_PC_PutString(), to be sure
...
void USBUART_PC_PutString(const char8 string[]) 
{
    ...
    strLength = USBUART_PC_StrLen(string);

    do
    {
        /* Limit length to maximum packet size of endpoint. */
        sendLength = (strLength > USBUART_PC_EP[epNumber].bufferSize) ?
                                  USBUART_PC_EP[epNumber].bufferSize : strLength;

        /* Load IN endpoint and expose it to host. */
        USBUART_PC_LoadInEP(epNumber, (const uint8 *)&string[bufIndex], sendLength);
        strLength -= sendLength;

        /* If more data are present to send or full packet was sent */
        if ((strLength > 0u) || (sendLength == USBUART_PC_EP[epNumber].bufferSize))
        {
            bufIndex += sendLength;

            /* Wait until host read data from IN endpoint buffer. */
            while (USBUART_PC_IN_BUFFER_FULL == USBUART_PC_EP[epNumber].apiEpState)
            {
            }
            ...
            */
            if (0u == strLength)
            {
                USBUART_PC_LoadInEP(epNumber, NULL, 0u);
            }
        }
    }
    while (strLength > 0u);
...
/***************************************************************************
* Function Name: USBUART_PC_PutChar
************************************************************************//**
*
...
void USBUART_PC_PutChar(char8 txDataByte) 
{
    ...
    USBUART_PC_LoadInEP(USBUART_PC_cdcDataInEp[USBUART_PC_activeCom], &dataByte, 1u);
}


/*******************************************************************************
* Function Name: USBUART_PC_PutCRLF
****************************************************************************//**
*
...
void USBUART_PC_PutCRLF(void) 
{
    ...
    USBUART_PC_LoadInEP(USBUART_PC_cdcDataInEp[USBUART_PC_activeCom], (const uint8 *)txData, 2u);
}


/*******************************************************************************
* Function Name: USBUART_PC_GetCount
****************************************************************************//**
*
*  This function returns the number of bytes that were received from the PC.
*  The returned length value should be passed to USBUART_PC_GetData() as
*  a parameter to read all received data. If all of the received data is not
*  read at one time by the USBUART_PC_GetData() API, the unread data will
*  be lost.
*
* \return
...
*
* \globalvars
*   USBUART_PC_cdcDataOutEp: CDC OUT endpoint number used.
*
*******************************************************************************/
uint16 USBUART_PC_GetCount(void) 
{
    ...
    uint8  epNumber = USBUART_PC_cdcDataOutEp[USBUART_PC_activeCom];

    if (USBUART_PC_OUT_BUFFER_FULL == USBUART_PC_EP[epNumber].apiEpState)
    {
        bytesCount = USBUART_PC_GetEPCount(epNumber);
    }
    else
    {
        bytesCount = 0u;
...
/*******************************************************************************
* Function Name: USBUART_PC_DataIsReady
****************************************************************************//**
*
*  This function returns a non-zero value if the component received data or
*  received zero-length packet. The USBUART_PC_GetAll() or
*  USBUART_PC_GetData() API should be called to read data from the buffer
...
{
    return (USBUART_PC_GetEPState(USBUART_PC_cdcDataOutEp[USBUART_PC_activeCom]));
}


/*******************************************************************************
* Function Name: USBUART_PC_CDCIsReady
****************************************************************************//**
*
...
*  USBUART_PC_PutData(),USBUART_PC_PutString(),
*  USBUART_PC_PutChar or USBUART_PC_PutCRLF(),
...
*
* \globalvars
*   USBUART_PC_cdcDataInEp: CDC IN endpoint number used.
*
*******************************************************************************/
uint8 USBUART_PC_CDCIsReady(void) 
{
    return (USBUART_PC_GetEPState(USBUART_PC_cdcDataInEp[USBUART_PC_activeCom]));
}


/***************************************************************************
* Function Name: USBUART_PC_GetData
************************************************************************//**
*
...
*  The USBUART_PC_DataIsReady() API should be called first, to be sure
*  that data is received from the host. If all received data will not be read at
*  once, the unread data will be lost. The USBUART_PC_GetData() API should
...
uint16 USBUART_PC_GetData(uint8* pData, uint16 length) 
{
    ...
    length = USBUART_PC_ReadOutEP(epNumber, pData, length);

#if (USBUART_PC_EP_MANAGEMENT_DMA_MANUAL)
    /* Wait until DMA complete transferring data from OUT endpoint buffer. */
    while (USBUART_PC_OUT_BUFFER_FULL == USBUART_PC_GetEPState(epNumber))
    {
    }
...
*
* \globalvars
*   - \ref USBUART_PC_cdcDataOutEp: CDC OUT endpoint number used.
*   - \ref USBUART_PC_EP[].bufferSize: EP max packet size is used as a
...
uint16 USBUART_PC_GetAll(uint8* pData) 
{
    ...
    /* Read data from OUT endpoint buffer. */
    dataLength = USBUART_PC_ReadOutEP(epNumber, pData, USBUART_PC_EP[epNumber].bufferSize);
...
/***************************************************************************
* Function Name: USBUART_PC_GetChar
************************************************************************//**
*
...
uint8 USBUART_PC_GetChar(void) 
{
    ...
    (void) USBUART_PC_ReadOutEP(epNumber, &rxData, 1u);
...
/*******************************************************************************
* Function Name: USBUART_PC_IsLineChanged
****************************************************************************//**
*
...
*  device. The USBUART_PC_GetDTERate(), USBUART_PC_GetCharFormat()
*  or USBUART_PC_GetParityType() or USBUART_PC_GetDataBits() API
*  should be called to read data coding information.
*  The USBUART_PC_GetLineControl() API should be called to read line
*  control information.
*
* \return
...
*
* \globalvars
*  - \ref USBUART_PC_transferState: it is checked to be sure then OUT
*    data phase has been complete, and data written to the lineCoding or
*    Control Bitmap buffer.
*  - \ref USBUART_PC_linesChanged: used as a flag to be aware that
...
uint8 USBUART_PC_IsLineChanged(void) 
{
    ...
    if (USBUART_PC_transferState == USBUART_PC_TRANS_STATE_IDLE)
    {
        if (USBUART_PC_linesChanged[USBUART_PC_activeCom] != 0u)
        {
            state = USBUART_PC_linesChanged[USBUART_PC_activeCom];
            USBUART_PC_linesChanged[USBUART_PC_activeCom] = 0u;
        }
    }
...
/***************************************************************************
* Function Name: USBUART_PC_GetDTERate
************************************************************************//**
*
...
*
* \globalvars
*  USBUART_PC_linesCoding: First four bytes converted to uint32
...
uint32 USBUART_PC_GetDTERate(void) 
{
    ...
    rate = USBUART_PC_linesCoding[USBUART_PC_activeCom][USBUART_PC_LINE_CODING_RATE + 3u];
    rate = (rate << 8u) | USBUART_PC_linesCoding[USBUART_PC_activeCom][USBUART_PC_LINE_CODING_RATE + 2u];
    rate = (rate << 8u) | USBUART_PC_linesCoding[USBUART_PC_activeCom][USBUART_PC_LINE_CODING_RATE + 1u];
    rate = (rate << 8u) | USBUART_PC_linesCoding[USBUART_PC_activeCom][USBUART_PC_LINE_CODING_RATE];
...
/*******************************************************************************
* Function Name: USBUART_PC_GetCharFormat
****************************************************************************//**
*
...
*
* \globalvars
*  USBUART_PC_linesCoding: used to get a parameter.
*
*******************************************************************************/
uint8 USBUART_PC_GetCharFormat(void) 
{
    return (USBUART_PC_linesCoding[USBUART_PC_activeCom][USBUART_PC_LINE_CODING_STOP_BITS]);
}


/*******************************************************************************
* Function Name: USBUART_PC_GetParityType
****************************************************************************//**
*
...
{
    return (USBUART_PC_linesCoding[USBUART_PC_activeCom][USBUART_PC_LINE_CODING_PARITY]);
}


/***************************************************************************
* Function Name: USBUART_PC_GetDataBits
************************************************************************//**
*
...
{
    return (USBUART_PC_linesCoding[USBUART_PC_activeCom][USBUART_PC_LINE_CODING_DATA_BITS]);
}


/***************************************************************************
* Function Name: USBUART_PC_GetLineControl
************************************************************************//**
*
...
*
* \globalvars
*  USBUART_PC_linesControlBitmap: used to get a parameter.
*
*******************************************************************************/
uint16 USBUART_PC_GetLineControl(void) 
{
    return (USBUART_PC_linesControlBitmap[USBUART_PC_activeCom]);
}


/*******************************************************************************
* Function Name: USBUART_PC_SendSerialState
****************************************************************************//**
*
...
*  USBUART_PC_NotificationIsReady() API must be called to check if the
...
{
    uint8 epNumber = USBUART_PC_cdcCommInInterruptEp[USBUART_PC_activeCom];

    if(USBUART_PC_SERIAL_STATE_SIZE <= USBUART_PC_EP[epNumber].bufferSize)
    {
        /* Save current SERIAL_STATE bitmap. */
        USBUART_PC_serialStateBitmap[USBUART_PC_activeCom] = serialState;

        /* Add interface number */
        USBUART_PC_serialStateNotification.wIndex = USBUART_PC_EP[epNumber].interface;

        /*Form SERIAL_STATE data*/
        USBUART_PC_serialStateNotification.wSerialState =    LO8(USBUART_PC_serialStateBitmap[USBUART_PC_activeCom]);
        USBUART_PC_serialStateNotification.wSerialStateMSB = HI8(USBUART_PC_serialStateBitmap[USBUART_PC_activeCom]);

        USBUART_PC_LoadInEP(epNumber, (uint8 *) &USBUART_PC_serialStateNotification, sizeof(USBUART_PC_serialStateNotification));
    }
}


/*******************************************************************************
* Function Name: USBUART_PC_GetSerialState
****************************************************************************//**
*
...
{
    return USBUART_PC_serialStateBitmap[USBUART_PC_activeCom];
}


/*******************************************************************************
* Function Name: USBUART_PC_NotificationIsReady
****************************************************************************//**
*
...
*  USBUART_PC_SendSerialState() to ensure that any previous
...
{
    return (USBUART_PC_EP[USBUART_PC_cdcCommInInterruptEp[USBUART_PC_activeCom]].apiEpState);
}


/*******************************************************************************
* Function Name: USBUART_PC_SetComPort
****************************************************************************//**
*
...
{
    if ((USBUART_PC_activeCom != comNumber) && \
            (comNumber < USBUART_PC_MAX_MULTI_COM_NUM ))
    {
        USBUART_PC_activeCom = comNumber;
    }
}


/*******************************************************************************
* Function Name: USBUART_PC_GetComPort
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USBUART_PC_Cdc_EpInit
************************************************************************//**
*
*  \internal
...
*   USBUART_PC_cdcDataInEp[], USBUART_PC_cdcCommInInterruptEp[],
*   USBUART_PC_cdcDataOutEp[]
*
...
uint8 USBUART_PC_Cdc_EpInit(const T_USBUART_PC_EP_SETTINGS_BLOCK CYCODE *pEP, uint8 epNum, uint8 cdcComNums) 
{
    ...
    {
        if (epType != USBUART_PC_EP_TYPE_INT)
        {
            USBUART_PC_cdcDataInEp[USBUART_PC_GET_EP_COM_NUM(cdcComNums, USBUART_PC_CDC_IN_EP)] = epNum;
            cdcComNums |= (uint8)(USBUART_PC_COM_PORT2 << USBUART_PC_CDC_IN_EP);
        }
        else
        {

            USBUART_PC_cdcCommInInterruptEp[USBUART_PC_GET_EP_COM_NUM(cdcComNums, USBUART_PC_CDC_NOTE_EP)] = epNum;
            cdcComNums |= (uint8)(USBUART_PC_COM_PORT2 << USBUART_PC_CDC_NOTE_EP);
        }
    }
    else
    {
        if (epType != USBUART_PC_EP_TYPE_INT)
        {
            USBUART_PC_cdcDataOutEp[USBUART_PC_GET_EP_COM_NUM(cdcComNums, USBUART_PC_CDC_OUT_EP)] = epNum;
            cdcComNums |= (uint8)(USBUART_PC_COM_PORT2 << USBUART_PC_CDC_OUT_EP);
        }
    }
    return (cdcComNums);
}
