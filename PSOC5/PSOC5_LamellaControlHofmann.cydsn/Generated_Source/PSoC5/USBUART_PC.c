...
* functions.
*/
uint8 USBUART_PC_initVar = 0u;

#if (USBUART_PC_EP_MANAGEMENT_DMA)
    #if (CY_PSOC4)
        static void USBUART_PC_InitEpDma(void);

        /* DMA chanels assigend for endpoints. */
        const uint8 USBUART_PC_DmaChan[USBUART_PC_MAX_EP] =
        {
            0u,
            0u,
            0u,
    ...
    #else
        /* DMA chanels assigend for endpoints. */
        uint8 USBUART_PC_DmaChan[USBUART_PC_MAX_EP];
        
        /* DMA TDs require for PSoC 3/5LP operation. */
        uint8 USBUART_PC_DmaTd[USBUART_PC_MAX_EP];
    #endif /* (CY_PSOC4) */
    ...
    uint8  USBUART_PC_DmaEpBurstCnt   [USBUART_PC_MAX_EP];
    
    /* Number of bytes to transfer in last DMA burst. */
    uint8  USBUART_PC_DmaEpLastBurstEl[USBUART_PC_MAX_EP];

    /* Storage for arrays above. */
    uint8  USBUART_PC_DmaEpBurstCntBackup  [USBUART_PC_MAX_EP];
    uint32 USBUART_PC_DmaEpBufferAddrBackup[USBUART_PC_MAX_EP];
     
    /* DMA trigger mux output for usb.dma_req[0-7]. */
    const uint8 USBUART_PC_DmaReqOut[USBUART_PC_MAX_EP] =
    {
        0u,
        USBUART_PC_ep1_dma__TR_OUTPUT,
        USBUART_PC_ep2_dma__TR_OUTPUT,
        USBUART_PC_ep3_dma__TR_OUTPUT,
        0u,
        0u,
        0u,
        0u,
    ...
    const uint8 USBUART_PC_DmaBurstEndOut[USBUART_PC_MAX_EP] =
    {
        0u,
        USBUART_PC_BURSTEND_0_TR_OUTPUT,
        USBUART_PC_BURSTEND_1_TR_OUTPUT,
        USBUART_PC_BURSTEND_2_TR_OUTPUT,
        USBUART_PC_BURSTEND_3_TR_OUTPUT,
        USBUART_PC_BURSTEND_4_TR_OUTPUT,
        USBUART_PC_BURSTEND_5_TR_OUTPUT,
        USBUART_PC_BURSTEND_6_TR_OUTPUT,
        USBUART_PC_BURSTEND_7_TR_OUTPUT
    };
    
#else
    #if (USBUART_PC_EP_DMA_AUTO_OPT == 0u)
        static uint8 clearInDataRdyStatus = USBUART_PC_ARB_EPX_CFG_DEFAULT;
        uint8 USBUART_PC_DmaNextTd[USBUART_PC_MAX_EP];
        const uint8 USBUART_PC_epX_TD_TERMOUT_EN[USBUART_PC_MAX_EP] =
        {
            0u,
            0u,
            0u,
        ...
        volatile uint16 USBUART_PC_inLength[USBUART_PC_MAX_EP];
        const uint8 *USBUART_PC_inDataPointer[USBUART_PC_MAX_EP];
        volatile uint8 USBUART_PC_inBufFull[USBUART_PC_MAX_EP];
...
/*******************************************************************************
* Function Name: USBUART_PC_Start
****************************************************************************//**
*
...
*   following list.
*
*       *USBUART_PC_3V_OPERATION* - Disable voltage regulator and pass-
*                                      through Vcc for pull-up
*
*       *USBUART_PC_5V_OPERATION* - Enable voltage regulator and use
*                                      regulator for pull-up
*
*       *USBUART_PC_DWR_POWER_OPERATION* - Enable or disable the voltage
...
{
    if (0u == USBUART_PC_initVar)
    {
        USBUART_PC_Init();
        USBUART_PC_initVar = 1u;
    }

    USBUART_PC_InitComponent(device, mode);
}


/*******************************************************************************
* Function Name: USBUART_PC_Init
****************************************************************************//**
*
...
* USBUART_PC_Init() because the USBUART_PC_Start() routine calls
...
void USBUART_PC_Init(void) 
{
    ...
    USBUART_PC_USB_CLK_EN_REG = USBUART_PC_USB_CLK_CSR_CLK_EN;

    /* The internal regulator (CR1.REG_ENABLE) is enabled in
    * USBUART_PC_InitComponent() if it is required.
    */

    /* Enable USBIO control on drive mode of D+ and D- pins. */
    USBUART_PC_USBIO_CR1_REG &= ~ (uint32) USBUART_PC_USBIO_CR1_IOMODE;

    /* Set number of LF CLK to detect UBS bus reset. */
    USBUART_PC_BUS_RST_CNT_REG = USBUART_PC_DEFUALT_BUS_RST_CNT;
    ...
    */
    USBUART_PC_POWER_CTRL_REG = USBUART_PC_DEFAULT_POWER_CTRL_VBUS;

    /* Enable PHY detector and single-ended and differential receivers. */
    USBUART_PC_POWER_CTRL_REG |= USBUART_PC_DEFAULT_POWER_CTRL_PHY;

    /* Suspend clear sequence. */
    USBUART_PC_POWER_CTRL_REG &= (uint32) ~USBUART_PC_POWER_CTRL_SUSPEND;
    CyDelayUs(USBUART_PC_WAIT_SUSPEND_DEL_DISABLE);
    USBUART_PC_POWER_CTRL_REG &= (uint32) ~USBUART_PC_POWER_CTRL_SUSPEND_DEL;

    /* Sets IMO lock options and clear all other bits. */
    USBUART_PC_CR1_REG = USBUART_PC_DEFUALT_CR1;

    /* Configure level (hi, lo, med) for each interrupt source. */
    USBUART_PC_INTR_LVL_SEL_REG = USBUART_PC_DEFAULT_INTR_LVL_SEL;

    /* Configure interrupt sources from: SOF, Bus Reset and EP0. */
    USBUART_PC_INTR_SIE_MASK_REG = USBUART_PC_DEFAULT_INTR_SIE_MASK;
    ...
    */
    USBUART_PC_PM_USB_CR0_REG &= (uint8) ~(USBUART_PC_PM_USB_CR0_PD_N |
                                                 USBUART_PC_PM_USB_CR0_PD_PULLUP_N);

    /* Select IOMODE to USB mode. */
    USBUART_PC_USBIO_CR1_REG &= (uint8) ~USBUART_PC_USBIO_CR1_IOMODE;
    ...
    CyDelayUs(USBUART_PC_WAIT_REG_STABILITY_1US);
    /* OR 40us after power is restored. */
    CyDelayUs(USBUART_PC_WAIT_VREF_RESTORE);
    /* Ensure single-ended disable bits are low (PRT15.INP_DIS[7:6])(input receiver enabled). */
    USBUART_PC_DM_INP_DIS_REG &= (uint8) ~USBUART_PC_DM_MASK;
    USBUART_PC_DP_INP_DIS_REG &= (uint8) ~USBUART_PC_DP_MASK;

    /* Enable USBIO. */
    USBUART_PC_PM_USB_CR0_REG |= USBUART_PC_PM_USB_CR0_PD_N;
    CyDelayUs(USBUART_PC_WAIT_PD_PULLUP_N_ENABLE);
    ...
    CY_SET_REG8(USBUART_PC_ARB_RW1_WA_PTR,     0u);
    CY_SET_REG8(USBUART_PC_ARB_RW1_WA_MSB_PTR, 0u);

#if (USBUART_PC_EP_MANAGEMENT_DMA)
    /* Initialize transfer descriptor. This will be used to detect DMA state - initialized or not. */
    for (i = 0u; i < USBUART_PC_MAX_EP; ++i)
    {
        USBUART_PC_DmaTd[i] = DMA_INVALID_TD;

    #if (USBUART_PC_EP_MANAGEMENT_DMA_AUTO && (USBUART_PC_EP_DMA_AUTO_OPT == 0u))
        USBUART_PC_DmaNextTd[i] = DMA_INVALID_TD;
    ...
    CyIntSetPriority     (USBUART_PC_INTR_HI_VECT_NUM,  USBUART_PC_INTR_HI_PRIORITY);
    (void) CyIntSetVector(USBUART_PC_INTR_HI_VECT_NUM, &USBUART_PC_INTR_HI_ISR);

    /* Configure lo_int: set handler and priority. */
    CyIntSetPriority     (USBUART_PC_INTR_LO_VECT_NUM,  USBUART_PC_INTR_LO_PRIORITY);
    (void) CyIntSetVector(USBUART_PC_INTR_LO_VECT_NUM, &USBUART_PC_INTR_LO_ISR);

    /* Configure med_int: set handler and priority (routed through DSI). */
    CyIntSetPriority     (USBUART_PC_INTR_MED_VECT_NUM,  USBUART_PC_INTR_MED_PRIORITY);
    (void) CyIntSetVector(USBUART_PC_INTR_MED_VECT_NUM, &USBUART_PC_INTR_MED_ISR);

#else
    /* Set bus reset interrupt. */
    CyIntSetPriority(USBUART_PC_BUS_RESET_VECT_NUM, USBUART_PC_BUS_RESET_PRIOR);
    (void) CyIntSetVector(USBUART_PC_BUS_RESET_VECT_NUM,   &USBUART_PC_BUS_RESET_ISR);

    /* Set Control Endpoint Interrupt. */
    CyIntSetPriority(USBUART_PC_EP_0_VECT_NUM, USBUART_PC_EP_0_PRIOR);
    (void) CyIntSetVector(USBUART_PC_EP_0_VECT_NUM,   &USBUART_PC_EP_0_ISR);

    /* Set SOF interrupt. */
    #if (USBUART_PC_SOF_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_SOF_VECT_NUM,  USBUART_PC_SOF_PRIOR);
        (void) CyIntSetVector(USBUART_PC_SOF_VECT_NUM, &USBUART_PC_SOF_ISR);
    #endif /* (USBUART_PC_SOF_ISR_ACTIVE) */

    /* Set Data Endpoint 1 Interrupt. */
    #if (USBUART_PC_EP1_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_EP_1_VECT_NUM,  USBUART_PC_EP_1_PRIOR);
        (void) CyIntSetVector(USBUART_PC_EP_1_VECT_NUM,  &USBUART_PC_EP_1_ISR);
    #endif /* (USBUART_PC_EP1_ISR_ACTIVE) */

    /* Set Data Endpoint 2 Interrupt. */
    #if (USBUART_PC_EP2_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_EP_2_VECT_NUM,  USBUART_PC_EP_2_PRIOR);
        (void) CyIntSetVector(USBUART_PC_EP_2_VECT_NUM, &USBUART_PC_EP_2_ISR);
    #endif /* (USBUART_PC_EP2_ISR_ACTIVE) */

    /* Set Data Endpoint 3 Interrupt. */
    #if (USBUART_PC_EP3_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_EP_3_VECT_NUM,  USBUART_PC_EP_3_PRIOR);
        (void) CyIntSetVector(USBUART_PC_EP_3_VECT_NUM, &USBUART_PC_EP_3_ISR);
    #endif /* (USBUART_PC_EP3_ISR_ACTIVE) */

    /* Set Data Endpoint 4 Interrupt. */
    #if (USBUART_PC_EP4_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_EP_4_VECT_NUM,  USBUART_PC_EP_4_PRIOR);
        (void) CyIntSetVector(USBUART_PC_EP_4_VECT_NUM, &USBUART_PC_EP_4_ISR);
    #endif /* (USBUART_PC_EP4_ISR_ACTIVE) */

    /* Set Data Endpoint 5 Interrupt. */
    #if (USBUART_PC_EP5_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_EP_5_VECT_NUM,  USBUART_PC_EP_5_PRIOR);
        (void) CyIntSetVector(USBUART_PC_EP_5_VECT_NUM, &USBUART_PC_EP_5_ISR);
    #endif /* (USBUART_PC_EP5_ISR_ACTIVE) */

    /* Set Data Endpoint 6 Interrupt. */
    #if (USBUART_PC_EP6_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_EP_6_VECT_NUM,  USBUART_PC_EP_6_PRIOR);
        (void) CyIntSetVector(USBUART_PC_EP_6_VECT_NUM, &USBUART_PC_EP_6_ISR);
    #endif /* (USBUART_PC_EP6_ISR_ACTIVE) */

     /* Set Data Endpoint 7 Interrupt. */
    #if (USBUART_PC_EP7_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_EP_7_VECT_NUM,  USBUART_PC_EP_7_PRIOR);
        (void) CyIntSetVector(USBUART_PC_EP_7_VECT_NUM, &USBUART_PC_EP_7_ISR);
    #endif /* (USBUART_PC_EP7_ISR_ACTIVE) */

    /* Set Data Endpoint 8 Interrupt. */
    #if (USBUART_PC_EP8_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_EP_8_VECT_NUM,  USBUART_PC_EP_8_PRIOR);
        (void) CyIntSetVector(USBUART_PC_EP_8_VECT_NUM, &USBUART_PC_EP_8_ISR);
    #endif /* (USBUART_PC_EP8_ISR_ACTIVE) */

    /* Set ARB Interrupt. */
    #if (USBUART_PC_EP_MANAGEMENT_DMA && USBUART_PC_ARB_ISR_ACTIVE)
        CyIntSetPriority     (USBUART_PC_ARB_VECT_NUM,  USBUART_PC_ARB_PRIOR);
        (void) CyIntSetVector(USBUART_PC_ARB_VECT_NUM, &USBUART_PC_ARB_ISR);
    #endif /* (USBUART_PC_EP_MANAGEMENT_DMA && USBUART_PC_ARB_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */

    /* Common: Configure GPIO interrupt for wakeup. */
#if (USBUART_PC_DP_ISR_ACTIVE)
    CyIntSetPriority     (USBUART_PC_DP_INTC_VECT_NUM,  USBUART_PC_DP_INTC_PRIORITY);
    (void) CyIntSetVector(USBUART_PC_DP_INTC_VECT_NUM, &USBUART_PC_DP_ISR);
#endif /* (USBUART_PC_DP_ISR_ACTIVE) */

#if (USBUART_PC_EP_MANAGEMENT_DMA && CY_PSOC4)
    /* Initialize DMA channels. */
    USBUART_PC_InitEpDma();
#endif /* (USBUART_PC_EP_MANAGEMENT_DMA && CY_PSOC4) */
}


/*******************************************************************************
* Function Name: USBUART_PC_InitComponent
****************************************************************************//**
*
...
*
* \globalvars
*   \ref USBUART_PC_device
*   \ref USBUART_PC_transferState
*   \ref USBUART_PC_configuration
*   \ref USBUART_PC_deviceStatus
*
*   \ref USBUART_PC_deviceAddress - Contains the current device address. This
...
*      SET_ADDRESS request.
*
*   \ref USBUART_PC_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
void USBUART_PC_InitComponent(uint8 device, uint8 mode) 
{
    ...
    */
    ...
    {
        USBUART_PC_hidProtocol[i] = USBUART_PC_PROTOCOL_REPORT;
    }
#endif /* USBUART_PC_ENABLE_HID_CLASS */

    /* Store device number to access descriptor. */
    USBUART_PC_device = device;

    /* Reset component internal variables. */
    USBUART_PC_transferState   = USBUART_PC_TRANS_STATE_IDLE;
    USBUART_PC_configurationChanged = 0u;
    USBUART_PC_configuration   = 0u;
    USBUART_PC_interfaceNumber = 0u;
    USBUART_PC_deviceAddress   = 0u;
    USBUART_PC_deviceStatus    = 0u;
    USBUART_PC_lastPacketSize  = 0u;

    /* Enable component interrupts. */
#if (CY_PSOC4)
    CyIntEnable(USBUART_PC_INTR_HI_VECT_NUM);
    CyIntEnable(USBUART_PC_INTR_MED_VECT_NUM);
    CyIntEnable(USBUART_PC_INTR_LO_VECT_NUM);
#else
    CyIntEnable(USBUART_PC_BUS_RESET_VECT_NUM);
    CyIntEnable(USBUART_PC_EP_0_VECT_NUM);

    #if (USBUART_PC_SOF_ISR_ACTIVE)
        CyIntEnable(USBUART_PC_SOF_VECT_NUM);
    #endif /* (USBUART_PC_SOF_ISR_ACTIVE) */

    #if (USBUART_PC_EP1_ISR_ACTIVE)
        CyIntEnable(USBUART_PC_EP_1_VECT_NUM);
    #endif /* (USBUART_PC_EP1_ISR_ACTIVE) */

    #if (USBUART_PC_EP2_ISR_ACTIVE)
        CyIntEnable(USBUART_PC_EP_2_VECT_NUM);
    #endif /* (USBUART_PC_EP5_ISR_ACTIVE) */

    #if (USBUART_PC_EP3_ISR_ACTIVE)
        CyIntEnable(USBUART_PC_EP_3_VECT_NUM);
    #endif /* (USBUART_PC_EP5_ISR_ACTIVE) */

    #if (USBUART_PC_EP4_ISR_ACTIVE)
        CyIntEnable(USBUART_PC_EP_4_VECT_NUM);
    #endif /* (USBUART_PC_EP5_ISR_ACTIVE) */

    #if (USBUART_PC_EP5_ISR_ACTIVE)
        CyIntEnable(USBUART_PC_EP_5_VECT_NUM);
    #endif /* (USBUART_PC_EP5_ISR_ACTIVE) */

    #if (USBUART_PC_EP6_ISR_ACTIVE)
        CyIntEnable(USBUART_PC_EP_6_VECT_NUM);
    #endif /* USBUART_PC_EP6_ISR_REMOVE */

    #if (USBUART_PC_EP7_ISR_ACTIVE)
        CyIntEnable(USBUART_PC_EP_7_VECT_NUM);
    #endif /* (USBUART_PC_EP7_ISR_ACTIVE) */

    #if (USBUART_PC_EP8_ISR_ACTIVE)
        CyIntEnable(USBUART_PC_EP_8_VECT_NUM);
    #endif /* (USBUART_PC_EP8_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */

#if (USBUART_PC_EP_MANAGEMENT_DMA && USBUART_PC_ARB_ISR_ACTIVE)
    /* Enable ARB EP interrupt sources. */
    USBUART_PC_ARB_INT_EN_REG = USBUART_PC_DEFAULT_ARB_INT_EN;

    #if (CY_PSOC3 || CY_PSOC5)
        CyIntEnable(USBUART_PC_ARB_VECT_NUM);
...
/* Arbiter configuration for DMA transfers. */
    ...
    USBUART_PC_ARB_CFG_REG = USBUART_PC_DEFAULT_ARB_CFG;
        ...
        CyDmaEnable();
            ...
            /* Change DMA priority to be highest. */
             CyIntSetPriority(CYDMA_INTR_NUMBER, USBUART_PC_DMA_AUTO_INTR_PRIO);
        #endif /* (USBUART_PC_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (CY_PSOC4) */
            ...
            CyIntEnable(CYDMA_INTR_NUMBER);
                ...
                USBUART_PC_EP_DMA_Done_isr_StartEx(&USBUART_PC_EP_DMA_DONE_ISR);
                USBUART_PC_EP17_DMA_Done_SR_InterruptEnable();
                USBUART_PC_EP8_DMA_Done_SR_InterruptEnable();
            #endif /* (USBUART_PC_EP_DMA_AUTO_OPT == 0u) */
        #endif /* (CY_PSOC4) */
    #endif /* (USBUART_PC_EP_MANAGEMENT_DMA_AUTO) */
#endif /* (USBUART_PC_EP_MANAGEMENT_DMA) */
    ...
    switch(mode)
    {
    case USBUART_PC_3V_OPERATION:
        /* Disable regulator for 3V operation. */
        USBUART_PC_CR1_REG &= (uint8) ~USBUART_PC_CR1_REG_ENABLE;
        break;

    case USBUART_PC_5V_OPERATION:
        /* Enable regulator for 5V operation. */
        USBUART_PC_CR1_REG |= (uint8)  USBUART_PC_CR1_REG_ENABLE;
        break;

    default: /* Check DWR settings of USB power supply. */
    #if (USBUART_PC_VDDD_MV < USBUART_PC_3500MV)
        /* Disable regulator for 3V operation. */
        USBUART_PC_CR1_REG &= (uint8) ~USBUART_PC_CR1_REG_ENABLE;
    #else
        /* Enable regulator for 5V operation. */
        USBUART_PC_CR1_REG |= (uint8)  USBUART_PC_CR1_REG_ENABLE;
    #endif /* (USBUART_PC_VDDD_MV < USBUART_PC_3500MV) */
        break;
    }

#if (CY_PSOC4)
    /* Clear bus activity. */
    USBUART_PC_CR1_REG &= (uint32) ~USBUART_PC_CR1_BUS_ACTIVITY;

    /* Clear EP0 count register. */
    USBUART_PC_EP0_CNT_REG = USBUART_PC_CLEAR_REG;

    /* Set EP0.CR: ACK Setup, NAK IN/OUT. */
    USBUART_PC_EP0_CR_REG = USBUART_PC_MODE_NAK_IN_OUT;

    #if (USBUART_PC_LPM_ACTIVE)
        if (NULL != USBUART_PC_GetBOSPtr())
        {
            /* Enable LPM and acknowledge LPM packets for active device.
            * Reset NYET_EN and SUB_RESP bits in the LPM_CTRL register.
            */
            USBUART_PC_LPM_CTRL_REG = (USBUART_PC_LPM_CTRL_LPM_EN | \
                                             USBUART_PC_LPM_CTRL_LPM_ACK_RESP);
        }
        else
        {
            /* Disable LPM for active device. */
            USBUART_PC_LPM_CTRL_REG &= (uint32) ~USBUART_PC_LPM_CTRL_LPM_EN;
        }
    #endif /* (USBUART_PC_LPM_ACTIVE) */

    /* Enable device to responds to USB traffic with address 0. */
    USBUART_PC_CR0_REG = USBUART_PC_DEFUALT_CR0;

#else
    /* Set EP0.CR: ACK Setup, STALL IN/OUT. */
    USBUART_PC_EP0_CR_REG = USBUART_PC_MODE_STALL_IN_OUT;

    /* Enable device to respond to USB traffic with address 0. */
    USBUART_PC_CR0_REG = USBUART_PC_DEFUALT_CR0;
    CyDelayCycles(USBUART_PC_WAIT_CR0_REG_STABILITY);
#endif /* (CY_PSOC4) */

    /* Enable D+ pull-up and keep USB control on IO. */
    USBUART_PC_USBIO_CR1_REG = USBUART_PC_USBIO_CR1_USBPUEN;
}


/*******************************************************************************
* Function Name: USBUART_PC_ReInitComponent
****************************************************************************//**
*
...
*
* \globalvars
*   USBUART_PC_device - Contains the device number of the desired Device
...
*   USBUART_PC_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
*   \ref USBUART_PC_deviceStatus
*
*   \ref USBUART_PC_configurationChanged
*
*   USBUART_PC_intiVar -  This variable is set to zero
*
*******************************************************************************/
void USBUART_PC_Stop(void) 
{
    ...
    USBUART_PC_Stop_DMA(USBUART_PC_MAX_EP);
#endif /* (USBUART_PC_EP_MANAGEMENT_DMA) */
    ...
    /* Disable D+ pull-up. */
    USBUART_PC_USBIO_CR1_REG &= (uint8) ~ USBUART_PC_USBIO_CR1_USBPUEN;

#if (CY_PSOC4)
    /* Disable USBFS block. */
    USBUART_PC_POWER_CTRL_REG &= (uint32) ~USBUART_PC_POWER_CTRL_ENABLE;
#else
...
#if (CY_PSOC4)
    CyIntDisable(USBUART_PC_INTR_HI_VECT_NUM);
    CyIntDisable(USBUART_PC_INTR_LO_VECT_NUM);
    CyIntDisable(USBUART_PC_INTR_MED_VECT_NUM);
#else

    CyIntDisable(USBUART_PC_BUS_RESET_VECT_NUM);
    CyIntDisable(USBUART_PC_EP_0_VECT_NUM);

    #if (USBUART_PC_SOF_ISR_ACTIVE)
        CyIntDisable(USBUART_PC_SOF_VECT_NUM);
    #endif /* (USBUART_PC_SOF_ISR_ACTIVE) */

    #if (USBUART_PC_EP1_ISR_ACTIVE)
        CyIntDisable(USBUART_PC_EP_1_VECT_NUM);
    #endif /* (USBUART_PC_EP1_ISR_ACTIVE) */

    #if (USBUART_PC_EP2_ISR_ACTIVE)
        CyIntDisable(USBUART_PC_EP_2_VECT_NUM);
    #endif /* (USBUART_PC_EP2_ISR_ACTIVE) */

    #if (USBUART_PC_EP3_ISR_ACTIVE)
        CyIntDisable(USBUART_PC_EP_3_VECT_NUM);
    #endif /* (USBUART_PC_EP3_ISR_ACTIVE) */

    #if (USBUART_PC_EP4_ISR_ACTIVE)
        CyIntDisable(USBUART_PC_EP_4_VECT_NUM);
    #endif /* (USBUART_PC_EP4_ISR_ACTIVE) */

    #if (USBUART_PC_EP5_ISR_ACTIVE)
        CyIntDisable(USBUART_PC_EP_5_VECT_NUM);
    #endif /* (USBUART_PC_EP5_ISR_ACTIVE) */

    #if (USBUART_PC_EP6_ISR_ACTIVE)
        CyIntDisable(USBUART_PC_EP_6_VECT_NUM);
    #endif /* USBUART_PC_EP6_ISR_REMOVE */

    #if (USBUART_PC_EP7_ISR_ACTIVE)
        CyIntDisable(USBUART_PC_EP_7_VECT_NUM);
    #endif /* (USBUART_PC_EP7_ISR_ACTIVE) */

    #if (USBUART_PC_EP8_ISR_ACTIVE)
        CyIntDisable(USBUART_PC_EP_8_VECT_NUM);
    #endif /* (USBUART_PC_EP8_ISR_ACTIVE) */

    #if (USBUART_PC_DP_ISR_ACTIVE)
        /* Clear active mode Dp interrupt source history. */
        (void) USBUART_PC_Dp_ClearInterrupt();
    ...
    USBUART_PC_initVar = 0u;
}


/*******************************************************************************
* Function Name: USBUART_PC_CheckActivity
****************************************************************************//**
*
...
{
    uint8 cr1Reg = USBUART_PC_CR1_REG;

    /* Clear bus activity. */
    USBUART_PC_CR1_REG = (cr1Reg & (uint8) ~USBUART_PC_CR1_BUS_ACTIVITY);

    /* Get bus activity. */
    return ((0u != (cr1Reg & USBUART_PC_CR1_BUS_ACTIVITY)) ? (1u) : (0u));
}


/*******************************************************************************
* Function Name: USBUART_PC_GetConfiguration
****************************************************************************//**
*
...
{
    return (USBUART_PC_configuration);
}


/*******************************************************************************
* Function Name: USBUART_PC_IsConfigurationChanged
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBUART_PC_configurationChanged - This variable is set to 1 after
...
uint8 USBUART_PC_IsConfigurationChanged(void) 
{
    ...
    if (USBUART_PC_configurationChanged != 0u)
    {
        res = USBUART_PC_configurationChanged;
        USBUART_PC_configurationChanged = 0u;
    }
...
/*******************************************************************************
* Function Name: USBUART_PC_GetInterfaceSetting
****************************************************************************//**
*
...
{
    return (USBUART_PC_interfaceSetting[interfaceNumber]);
}


/*******************************************************************************
* Function Name: USBUART_PC_GetEPState
****************************************************************************//**
*
...
{
    return (USBUART_PC_EP[epNumber].apiEpState);
}


/*******************************************************************************
* Function Name: USBUART_PC_GetEPCount
****************************************************************************//**
*
...
uint16 USBUART_PC_GetEPCount(uint8 epNumber) 
{
    ...
    if ((epNumber > USBUART_PC_EP0) && (epNumber < USBUART_PC_MAX_EP))
    {
        ...
        cntr  = ((uint16) USBUART_PC_SIE_EP_BASE.sieEp[epNumber].epCnt0) & USBUART_PC_EPX_CNT0_MASK;
        cntr  = ((uint16) (cntr << 8u)) | ((uint16) USBUART_PC_SIE_EP_BASE.sieEp[epNumber].epCnt1);
        cntr -= USBUART_PC_EPX_CNTX_CRC_COUNT;
    }
...
#if (CY_PSOC4)
    /*******************************************************************************
    * Function Name: USBUART_PC_InitEpDma
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    static void USBUART_PC_InitEpDma(void)
    {
        ...
        CYDMA_CH_CTL_BASE.ctl[USBUART_PC_ep1_dma_CHANNEL] = USBUART_PC_ep1_dma_CHANNEL_CFG;
    #endif /* (USBUART_PC_DMA1_ACTIVE) */

    #if (USBUART_PC_DMA2_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART_PC_ep2_dma_CHANNEL] = USBUART_PC_ep2_dma_CHANNEL_CFG;
    #endif /* (USBUART_PC_DMA2_ACTIVE) */

    #if (USBUART_PC_DMA3_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART_PC_ep3_dma_CHANNEL] = USBUART_PC_ep3_dma_CHANNEL_CFG;
    #endif /* (USBUART_PC_DMA3_ACTIVE) */

    #if (USBUART_PC_DMA4_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART_PC_ep4_dma_CHANNEL] = USBUART_PC_ep4_dma_CHANNEL_CFG;
    #endif /* (USBUART_PC_DMA4_ACTIVE) */

    #if (USBUART_PC_DMA5_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART_PC_ep5_dma_CHANNEL] = USBUART_PC_ep5_dma_CHANNEL_CFG;
    #endif /* (USBUART_PC_DMA5_ACTIVE) */

    #if (USBUART_PC_DMA6_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART_PC_ep6_dma_CHANNEL] = USBUART_PC_ep6_dma_CHANNEL_CFG;
    #endif /* (USBUART_PC_DMA6_ACTIVE) */

    #if (USBUART_PC_DMA7_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART_PC_ep7_dma_CHANNEL] = USBUART_PC_ep7_dma_CHANNEL_CFG;
    #endif /* (USBUART_PC_DMA7_ACTIVE) */

    #if (USBUART_PC_DMA8_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[USBUART_PC_ep8_dma_CHANNEL] = USBUART_PC_ep8_dma_CHANNEL_CFG;
    #endif /* (USBUART_PC_DMA8_ACTIVE) */

    #if (USBUART_PC_EP_MANAGEMENT_DMA_AUTO)
        /* Initialize DMA channel callbacks. */
        #if (USBUART_PC_DMA1_ACTIVE)
            (void) USBUART_PC_ep1_dma_SetInterruptCallback(&USBUART_PC_EP1_DMA_DONE_ISR);
        #endif /* (USBUART_PC_DMA1_ACTIVE) */

        #if (USBUART_PC_DMA2_ACTIVE)
            (void) USBUART_PC_ep2_dma_SetInterruptCallback(&USBUART_PC_EP2_DMA_DONE_ISR);
        #endif /* (USBUART_PC_DMA2_ACTIVE) */

        #if (USBUART_PC_DMA3_ACTIVE)
            (void) USBUART_PC_ep3_dma_SetInterruptCallback(&USBUART_PC_EP3_DMA_DONE_ISR);
        #endif /* (USBUART_PC_DMA3_ACTIVE) */

        #if (USBUART_PC_DMA4_ACTIVE)
            (void) USBUART_PC_ep4_dma_SetInterruptCallback(&USBUART_PC_EP4_DMA_DONE_ISR);
        #endif /* (USBUART_PC_DMA4_ACTIVE) */

        #if (USBUART_PC_DMA5_ACTIVE)
            (void) USBUART_PC_ep5_dma_SetInterruptCallback(&USBUART_PC_EP5_DMA_DONE_ISR);
        #endif /* (USBUART_PC_DMA5_ACTIVE) */

        #if (USBUART_PC_DMA6_ACTIVE)
            (void) USBUART_PC_ep6_dma_SetInterruptCallback(&USBUART_PC_EP6_DMA_DONE_ISR);
        #endif /* (USBUART_PC_DMA6_ACTIVE) */

        #if (USBUART_PC_DMA7_ACTIVE)
            (void) USBUART_PC_ep7_dma_SetInterruptCallback(&USBUART_PC_EP7_DMA_DONE_ISR);
        #endif /* (USBUART_PC_DMA7_ACTIVE) */

        #if (USBUART_PC_DMA8_ACTIVE)
            (void) USBUART_PC_ep8_dma_SetInterruptCallback(&USBUART_PC_EP8_DMA_DONE_ISR);
        #endif /* (USBUART_PC_DMA8_ACTIVE) */
    #endif /* (USBUART_PC_EP_MANAGEMENT_DMA_AUTO) */
    }
#else
    ...
    /***************************************************************************
    * Function Name: USBUART_PC_InitEP_DMA
    ************************************************************************//**
    *
    *  This function allocates and initializes a DMA channel to be used by the
    *  USBUART_PC_LoadInEP() or USBUART_PC_ReadOutEP() APIs for data
    ...
    ***************************************************************************/
                                                                    ...
                                                                    
    {
    ...
    #if (USBUART_PC_DMA1_ACTIVE)
        case USBUART_PC_EP1:
            USBUART_PC_DmaChan[epNumber] = USBUART_PC_ep1_DmaInitialize(USBUART_PC_DMA_BYTES_PER_BURST,
                                                                                    USBUART_PC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART_PC_DMA1_ACTIVE) */

    #if (USBUART_PC_DMA2_ACTIVE)
        case USBUART_PC_EP2:
            USBUART_PC_DmaChan[epNumber] = USBUART_PC_ep2_DmaInitialize(USBUART_PC_DMA_BYTES_PER_BURST,
                                                                                    USBUART_PC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART_PC_DMA2_ACTIVE) */
    ...
    #if (USBUART_PC_DMA3_ACTIVE)
            case USBUART_PC_EP3:
                USBUART_PC_DmaChan[epNumber] = USBUART_PC_ep3_DmaInitialize(USBUART_PC_DMA_BYTES_PER_BURST,
                                                                                        USBUART_PC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART_PC_DMA3_ACTIVE) */
    ...
    #if (USBUART_PC_DMA4_ACTIVE)
            case USBUART_PC_EP4:
                USBUART_PC_DmaChan[epNumber] = USBUART_PC_ep4_DmaInitialize(USBUART_PC_DMA_BYTES_PER_BURST,
                                                                                        USBUART_PC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART_PC_DMA4_ACTIVE) */
    ...
    #if (USBUART_PC_DMA5_ACTIVE)
            case USBUART_PC_EP5:
                USBUART_PC_DmaChan[epNumber] = USBUART_PC_ep5_DmaInitialize(USBUART_PC_DMA_BYTES_PER_BURST,
                                                                                        USBUART_PC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART_PC_DMA5_ACTIVE) */
    ...
    #if (USBUART_PC_DMA6_ACTIVE)
        case USBUART_PC_EP6:
            USBUART_PC_DmaChan[epNumber] = USBUART_PC_ep6_DmaInitialize(USBUART_PC_DMA_BYTES_PER_BURST,
                                                                                    USBUART_PC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART_PC_DMA6_ACTIVE) */
    ...
    #if (USBUART_PC_DMA7_ACTIVE)
        case USBUART_PC_EP7:
                USBUART_PC_DmaChan[epNumber] = USBUART_PC_ep7_DmaInitialize(USBUART_PC_DMA_BYTES_PER_BURST,
                                                                                        USBUART_PC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART_PC_DMA7_ACTIVE) */
    ...
    #if (USBUART_PC_DMA8_ACTIVE)
        case USBUART_PC_EP8:
                USBUART_PC_DmaChan[epNumber] = USBUART_PC_ep8_DmaInitialize(USBUART_PC_DMA_BYTES_PER_BURST,
                                                                                        USBUART_PC_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (USBUART_PC_DMA8_ACTIVE) */
        ...
        if ((epNumber > USBUART_PC_EP0) && (epNumber < USBUART_PC_MAX_EP))
        {
            USBUART_PC_DmaTd[epNumber] = CyDmaTdAllocate();

        #if (USBUART_PC_EP_MANAGEMENT_DMA_AUTO && (USBUART_PC_EP_DMA_AUTO_OPT == 0u))
            USBUART_PC_DmaNextTd[epNumber] = CyDmaTdAllocate();
        #endif /* (USBUART_PC_EP_MANAGEMENT_DMA_AUTO && (USBUART_PC_EP_DMA_AUTO_OPT == 0u)) */
        }
    }
#endif /* (CY_PSOC4) */
    ...
    /***************************************************************************
    * Function Name: USBUART_PC_Stop_DMA
    ************************************************************************//**
    *  
    ...
    *  to trigger DMA re-configuration when USBUART_PC_LoadInEP() or 
    *  USBUART_PC_ReadOutEP() functions are called the first time. 
    *  
    ...
    ***************************************************************************/
    void USBUART_PC_Stop_DMA(uint8 epNumber) 
    {
        ...
        i = (epNumber < USBUART_PC_MAX_EP) ? epNumber : USBUART_PC_EP1;

        do
        {
        #if (CY_PSOC4)
            if (0u != USBUART_PC_DmaChan[i])
            {
                USBUART_PC_CyDmaChDisable(USBUART_PC_DmaChan[i]);
            }
        #else
            if(USBUART_PC_DmaTd[i] != DMA_INVALID_TD)
            {
                (void) CyDmaChDisable(USBUART_PC_DmaChan[i]);
                CyDmaTdFree(USBUART_PC_DmaTd[i]);
                USBUART_PC_DmaTd[i] = DMA_INVALID_TD;
            }

            #if (USBUART_PC_EP_MANAGEMENT_DMA_AUTO && (USBUART_PC_EP_DMA_AUTO_OPT == 0u))
                if(USBUART_PC_DmaNextTd[i] != DMA_INVALID_TD)
                {
                    CyDmaTdFree(USBUART_PC_DmaNextTd[i]);
                    USBUART_PC_DmaNextTd[i] = DMA_INVALID_TD;
                }
            #endif /* (USBUART_PC_EP_MANAGEMENT_DMA_AUTO && (USBUART_PC_EP_DMA_AUTO_OPT == 0u)) */
        ...
        while ((i < USBUART_PC_MAX_EP) && (epNumber == USBUART_PC_MAX_EP));
    }
#endif /* (USBUART_PC_EP_MANAGEMENT_DMA) */
...
#if (USBUART_PC_EP_MANAGEMENT_DMA_AUTO && (USBUART_PC_EP_DMA_AUTO_OPT == 0u))
    /***************************************************************************
    * Function Name: USBUART_PC_LoadNextInEP
    ************************************************************************//**
    *
    * Summary:
    ...
    ***************************************************************************/
    void USBUART_PC_LoadNextInEP(uint8 epNumber, uint8 mode) 
    {
        ...
        if (mode == 0u)
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBUART_PC_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USBUART_PC_inLength[epNumber] - USBUART_PC_DMA_BYTES_PER_BURST);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBUART_PC_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert, LO16((uint32)USBUART_PC_inDataPointer[epNumber] +
                                            USBUART_PC_DMA_BYTES_PER_BURST));
            USBUART_PC_inBufFull[epNumber] = 1u;
        }
        else
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBUART_PC_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, USBUART_PC_DMA_BYTES_REPEAT);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[USBUART_PC_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert,  LO16((uint32)USBUART_PC_inDataPointer[epNumber] +
                                   (USBUART_PC_DMA_BYTES_PER_BURST - USBUART_PC_DMA_BYTES_REPEAT)));
        }

        /* CyDmaChSetInitialTd API is optimized to initialize TD. */
        CY_DMA_CH_STRUCT_PTR[USBUART_PC_DmaChan[epNumber]].basic_status[1u] = USBUART_PC_DmaTd[epNumber];
    }
#endif /* (USBUART_PC_EP_MANAGEMENT_DMA_AUTO && (USBUART_PC_EP_DMA_AUTO_OPT == 0u)) */
#endif /* (CY_PSOC3 || CY_PSOC5) */
...
/*******************************************************************************
* Function Name: USBUART_PC_LoadInEP
****************************************************************************//**
*
                                                                        ...
                                                                        
{
    if ((epNumber > USBUART_PC_EP0) && (epNumber < USBUART_PC_MAX_EP))
    {
        ...
        if (length > (USBUART_PC_EPX_DATA_BUF_MAX - USBUART_PC_EP[epNumber].buffOffset))
        {
            length = USBUART_PC_EPX_DATA_BUF_MAX - USBUART_PC_EP[epNumber].buffOffset;
        }
    #endif /* (!USBUART_PC_EP_MANAGEMENT_DMA_AUTO) */

        /* Set count and data toggle. */
        USBUART_PC_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint8) HI8(length) | USBUART_PC_EP[epNumber].epToggle;
        USBUART_PC_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint8) LO8(length);

    #if (USBUART_PC_EP_MANAGEMENT_MANUAL)
        if (NULL != pData)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr = pData[i];
            }
        }

        /* IN endpoint buffer is full - read to be read. */
        USBUART_PC_EP[epNumber].apiEpState = USBUART_PC_NO_EVENT_PENDING;

        /* Arm IN endpoint. */
        USBUART_PC_SIE_EP_BASE.sieEp[epNumber].epCr0 = USBUART_PC_EP[epNumber].epMode;

    #else

        #if (CY_PSOC3 || CY_PSOC5LP)
            /* Initialize DMA if it was not initialized. */
            if (DMA_INVALID_TD == USBUART_PC_DmaTd[epNumber])
            {
                USBUART_PC_InitEP_DMA(epNumber, pData);
            }
            ...
            {
            #if (CY_PSOC4)
                uint32 channelNum = (uint32) USBUART_PC_DmaChan[epNumber];

                /* Disable DMA channel: start configuration. */
                USBUART_PC_CyDmaChDisable(channelNum);

                /* Configure source and destination. */
                USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) pData);
                USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                --length;
                USBUART_PC_CyDmaSetConfiguration(channelNum, USBUART_PC_DMA_DESCR0, USBUART_PC_DMA_COMMON_CFG | length |
                ...
                (void) CyDmaChDisable(USBUART_PC_DmaChan[epNumber]);
                (void) CyDmaTdSetConfiguration(USBUART_PC_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(USBUART_PC_DmaTd[epNumber], LO16((uint32) pData), LO16((uint32) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr));

                /* Enable DMA channel. */
                (void) CyDmaChSetInitialTd(USBUART_PC_DmaChan[epNumber], USBUART_PC_DmaTd[epNumber]);
                (void) CyDmaChEnable(USBUART_PC_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */

                /* Generate DMA request. */
                USBUART_PC_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint8)  USBUART_PC_ARB_EPX_CFG_DMA_REQ;
                USBUART_PC_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint8) ~USBUART_PC_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                USBUART_PC_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
                ...
                USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr);
                USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR1, (void*) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                USBUART_PC_CyDmaSetConfiguration(channelNum, USBUART_PC_DMA_DESCR0, USBUART_PC_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Configure DMA descriptor. */
                USBUART_PC_CyDmaSetConfiguration(channelNum, USBUART_PC_DMA_DESCR1, USBUART_PC_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Enable interrupt from DMA channel. */
                USBUART_PC_CyDmaSetInterruptMask(channelNum);
                ...
                (void) CyDmaChDisable(USBUART_PC_DmaChan[epNumber]);
                ...
                #if (USBUART_PC_EP_DMA_AUTO_OPT == 0u)
                    USBUART_PC_inLength[epNumber] = length;
                    USBUART_PC_inDataPointer[epNumber] = pData;

                    /* Configure DMA to send data only for first burst */
                    (void) CyDmaTdSetConfiguration(USBUART_PC_DmaTd[epNumber],
                        (length > USBUART_PC_DMA_BYTES_PER_BURST) ? USBUART_PC_DMA_BYTES_PER_BURST : length,
                        USBUART_PC_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USBUART_PC_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    ...
                    */
                    (void) CyDmaTdSetConfiguration(USBUART_PC_DmaNextTd[epNumber], 1u,
                                                   USBUART_PC_DmaNextTd[epNumber],
                                                   USBUART_PC_epX_TD_TERMOUT_EN[epNumber]);

                    /* Configure DmaNextTd to clear Data Ready status. */
                    (void) CyDmaTdSetAddress(USBUART_PC_DmaNextTd[epNumber], LO16((uint32) &clearInDataRdyStatus),
                                                                                   LO16((uint32) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].epCfg));
                #else
                    /* Configure DMA to send all data. */
                    (void) CyDmaTdSetConfiguration(USBUART_PC_DmaTd[epNumber], length,
                                                   USBUART_PC_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(USBUART_PC_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr));
                #endif /* (USBUART_PC_EP_DMA_AUTO_OPT == 0u) */

                /* Clear any potential pending DMA requests before starting DMA channel to transfer data. */
                (void) CyDmaClearPendingDrq(USBUART_PC_DmaChan[epNumber]);
                /* Enable DMA. */
                (void) CyDmaChSetInitialTd(USBUART_PC_DmaChan[epNumber], USBUART_PC_DmaTd[epNumber]);
                (void) CyDmaChEnable(USBUART_PC_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    USBUART_PC_DmaEpBurstCnt[epNumber] = (uint8) (length / USBUART_PC_DMA_BYTES_PER_BURST);

                    /* Get number of elements in the last burst. */
                    USBUART_PC_DmaEpLastBurstEl[epNumber] = (uint8) (length % USBUART_PC_DMA_BYTES_PER_BURST);

                    /* Get total number of bursts. */
                    USBUART_PC_DmaEpBurstCnt[epNumber] += (0u != USBUART_PC_DmaEpLastBurstEl[epNumber]) ? 1u : 0u;

                    /* Adjust number of data elements transferred in last burst. */
                    USBUART_PC_DmaEpLastBurstEl[epNumber] = (0u != USBUART_PC_DmaEpLastBurstEl[epNumber]) ?
                                                                          (USBUART_PC_DmaEpLastBurstEl[epNumber] - 1u) :
                                                                          (USBUART_PC_DMA_BYTES_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USBUART_PC_DmaEpBurstCnt[epNumber]) ? USBUART_PC_DmaEpLastBurstEl[epNumber] : (USBUART_PC_DMA_BYTES_PER_BURST - 1u);
                    lengthDescr1 = (2u == USBUART_PC_DmaEpBurstCnt[epNumber]) ? USBUART_PC_DmaEpLastBurstEl[epNumber] : (USBUART_PC_DMA_BYTES_PER_BURST - 1u);


                    /* Mark which descriptor is last one to execute. */
                    USBUART_PC_DmaEpLastBurstEl[epNumber] |= (0u != (USBUART_PC_DmaEpBurstCnt[epNumber] & 0x1u)) ?
                                                                            USBUART_PC_DMA_DESCR0_MASK : USBUART_PC_DMA_DESCR1_MASK;

                    /* Restore DMA settings for current transfer. */
                    USBUART_PC_CyDmaChDisable(channelNum);

                    /* Restore destination address for input endpoint. */
                    USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) ((uint32) USBUART_PC_DmaEpBufferAddrBackup[epNumber]));
                    USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR1, (void*) ((uint32) USBUART_PC_DmaEpBufferAddrBackup[epNumber] +
                                                                                                                   USBUART_PC_DMA_BYTES_PER_BURST));

                    /* Set number of elements to transfer. */
                    USBUART_PC_CyDmaSetNumDataElements(channelNum, USBUART_PC_DMA_DESCR0, lengthDescr0);
                    USBUART_PC_CyDmaSetNumDataElements(channelNum, USBUART_PC_DMA_DESCR1, lengthDescr1);
                    ...
                    if (USBUART_PC_DmaEpBurstCnt[epNumber] > 1u)
                    {
                    ...
                    USBUART_PC_DmaEpBurstCnt[epNumber] = USBUART_PC_DMA_GET_BURST_CNT(USBUART_PC_DmaEpBurstCnt[epNumber]);

                    /* Enable DMA channel: configuration complete. */
                    USBUART_PC_CyDmaChEnable(channelNum);

                #elif (USBUART_PC_EP_DMA_AUTO_OPT == 0u)
                    USBUART_PC_inLength[epNumber]  = length;
                    USBUART_PC_inBufFull[epNumber] = 0u;

                    (void) CyDmaChDisable(USBUART_PC_DmaChan[epNumber]);
                    /* Configure DMA to send data only for first burst. */
                    (void) CyDmaTdSetConfiguration(
                        USBUART_PC_DmaTd[epNumber], (length > USBUART_PC_DMA_BYTES_PER_BURST) ?
                        USBUART_PC_DMA_BYTES_PER_BURST : length,
                        USBUART_PC_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR );
                    (void) CyDmaTdSetAddress(USBUART_PC_DmaTd[epNumber], LO16((uint32)  USBUART_PC_inDataPointer[epNumber]),
                                                                               LO16((uint32) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    /* Clear Any potential pending DMA requests before starting DMA channel to transfer data. */
                    (void) CyDmaClearPendingDrq(USBUART_PC_DmaChan[epNumber]);
                    /* Enable DMA. */
                ...
                #if !defined (USBUART_PC_MANUAL_IN_EP_ARM)
                    /* Set IN data ready to generate DMA request to load data into endpoint buffer. */
                    USBUART_PC_ARB_EP_BASE.arbEp[epNumber].epCfg |= USBUART_PC_ARB_EPX_CFG_IN_DATA_RDY;
                #endif  /* (USBUART_PC_MANUAL_IN_EP_ARM) */
        ...
        #endif /* (USBUART_PC_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (USBUART_PC_EP_MANAGEMENT_MANUAL) */
    }
}


/*******************************************************************************
* Function Name: USBUART_PC_ReadOutEP
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
...
*   transfer after calling the USBUART_PC_ReadOutEP() API. For example,
*   by checking EPstate:
*
...
{
    if ((pData != NULL) && (epNumber > USBUART_PC_EP0) && (epNumber < USBUART_PC_MAX_EP))
    {
    #if (!USBUART_PC_EP_MANAGEMENT_DMA_AUTO)
        /* Adjust requested length to available data. */
        length = (length > USBUART_PC_GetEPCount(epNumber)) ? USBUART_PC_GetEPCount(epNumber) : length;
    #endif /* (!USBUART_PC_EP_MANAGEMENT_DMA_AUTO) */
    ...
    #if (USBUART_PC_EP_MANAGEMENT_MANUAL)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                pData[i] = (uint8) USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr;
            }
        }

        /* Arm OUT endpoint after data has been copied from endpoint buffer. */
        USBUART_PC_EnableOutEP(epNumber);
    #else
        ...
        #if (CY_PSOC4)
        {
            ...
            /* Configure source and destination. */
            USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USBUART_PC_CyDmaSetConfiguration(channelNum, USBUART_PC_DMA_DESCR0, USBUART_PC_DMA_COMMON_CFG | (uint16)(length - 1u) |
            ...
            /* Configure DMA to transfer data. */
            (void) CyDmaChDisable(USBUART_PC_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USBUART_PC_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USBUART_PC_DmaTd[epNumber], LO16((uint32) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32)pData));
        ...
        #if (CY_PSOC4)
        {
            ...
            USBUART_PC_DmaEpLastBurstEl[epNumber] |= (USBUART_PC_DmaEpBurstCnt[epNumber] > 2u) ? USBUART_PC_DMA_DESCR_REVERT : 0u;
            ...
            USBUART_PC_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
            USBUART_PC_DmaEpBurstCntBackup[epNumber]   = USBUART_PC_DmaEpBurstCnt[epNumber];
            ...
            /* Set destination address. */
            USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr);
            USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR1, (void*) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr);

            USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) ((uint32) pData));
            USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR1, (void*) ((uint32) pData + USBUART_PC_DMA_BYTES_PER_BURST));

            /* Configure DMA descriptor. */
            USBUART_PC_CyDmaSetConfiguration(channelNum, USBUART_PC_DMA_DESCR0, USBUART_PC_DMA_COMMON_CFG  | lengthDescr0 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

            /* Configure DMA descriptor. */
            USBUART_PC_CyDmaSetConfiguration(channelNum, USBUART_PC_DMA_DESCR1, USBUART_PC_DMA_COMMON_CFG  | lengthDescr1 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);
            ...
            if (USBUART_PC_DmaEpBurstCntBackup[epNumber] > 1u)
            {
                USBUART_PC_CyDmaValidateDescriptor(channelNum, USBUART_PC_DMA_DESCR1);
            }
        ...
        #else
            (void) CyDmaChDisable(USBUART_PC_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USBUART_PC_DmaTd[epNumber], length,  USBUART_PC_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USBUART_PC_DmaTd[epNumber], LO16((uint32) &USBUART_PC_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32) pData));
...
/*******************************************************************************
* Function Name: USBUART_PC_LoadInEP16
****************************************************************************//**
*
*  This function performs different functionality depending on the Component’s
*  configured Endpoint Buffer Management. This parameter is defined in
...
void USBUART_PC_LoadInEP16(uint8 epNumber, const uint8 pData[], uint16 length)
{
    ...
    if ((epNumber > USBUART_PC_EP0) && (epNumber < USBUART_PC_MAX_EP))
    {
        ...
        /* Set count and data toggle. */
        USBUART_PC_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint32) HI8(length) | USBUART_PC_EP[epNumber].epToggle;
        USBUART_PC_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint32) LO8(length);
        ...
        if (NULL != pData)
        {
            ...
            {
                USBUART_PC_ARB_EP16_BASE.arbEp[epNumber].rwDr16 = dataBuf[i];
            }
        }
        ...
        USBUART_PC_SIE_EP_BASE.sieEp[epNumber].epCr0 = USBUART_PC_EP[epNumber].epMode;
            ...
            if ((pData != NULL) && (length > 0u))
            {
                ...
                /* Configure source and destination. */
                USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) pData);
                USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) &USBUART_PC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                ...
                /* Generate DMA request. */
                USBUART_PC_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint32)  USBUART_PC_ARB_EPX_CFG_DMA_REQ;
                USBUART_PC_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint32) ~USBUART_PC_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                /* Set destination address. */
                USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) &USBUART_PC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR1, (void*) &USBUART_PC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    /* Adjust number of data elements transferred in last burst. */
                    USBUART_PC_DmaEpLastBurstEl[epNumber] = (0u != USBUART_PC_DmaEpLastBurstEl[epNumber]) ?
                                                                          ((USBUART_PC_DmaEpLastBurstEl[epNumber] >> 1u) - 1u) :
                                                                           (USBUART_PC_DMA_HALFWORDS_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == USBUART_PC_DmaEpBurstCnt[epNumber]) ? USBUART_PC_DmaEpLastBurstEl[epNumber] : (USBUART_PC_DMA_HALFWORDS_PER_BURST - 1u);
                    lengthDescr1 = (2u == USBUART_PC_DmaEpBurstCnt[epNumber]) ? USBUART_PC_DmaEpLastBurstEl[epNumber] : (USBUART_PC_DMA_HALFWORDS_PER_BURST - 1u);
...
/*******************************************************************************
* Function Name: USBUART_PC_ReadOutEP16
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
*   configured Endpoint Buffer Management. This parameter is defined in the
...
uint16 USBUART_PC_ReadOutEP16(uint8 epNumber, uint8 pData[], uint16 length)
{
    ...
    adjLength =  length + ((uint32)length & 1u);
    ...
    #if (USBUART_PC_EP_MANAGEMENT_MANUAL)
        {
            ...
            {
                dataBuf[i] = (uint16) USBUART_PC_ARB_EP16_BASE.arbEp[epNumber].rwDr16;
            }
        }
        ...
        #if (USBUART_PC_EP_MANAGEMENT_DMA_MANUAL)
        {
            ...
            /* Configure source and destination. */
            USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) &USBUART_PC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USBUART_PC_CyDmaSetDstAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            USBUART_PC_CyDmaSetConfiguration(channelNum, USBUART_PC_DMA_DESCR0, USBUART_PC_DMA_COMMON_CFG | (uint16)((adjLength >> 1u) - 1u) |
        ...
        #if (USBUART_PC_EP_MANAGEMENT_DMA_AUTO)
        {
            ...
            /* Get number of full bursts. */
            USBUART_PC_DmaEpBurstCnt[epNumber] = (uint8) (adjLength / USBUART_PC_DMA_BYTES_PER_BURST);

            /* Get number of elements in last burst. */
            USBUART_PC_DmaEpLastBurstEl[epNumber] = (uint8) (adjLength % USBUART_PC_DMA_BYTES_PER_BURST);
            ...
            USBUART_PC_DmaEpLastBurstEl[epNumber] |= USBUART_PC_DMA_DESCR_16BITS;
            ...
            /* Set destination address. */
            USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR0, (void*) &USBUART_PC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            USBUART_PC_CyDmaSetSrcAddress(channelNum, USBUART_PC_DMA_DESCR1, (void*) &USBUART_PC_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
...
/*******************************************************************************
* Function Name: USBUART_PC_EnableOutEP
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBUART_PC_EP[epNumber].apiEpState - set to NO_EVENT_PENDING
*
* \reentrant
*  No.
*
...
/*******************************************************************************
* Function Name: USBUART_PC_DisableOutEP
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: USBUART_PC_Force
****************************************************************************//**
*
...
*    State                      |Description
*    ---------------------------|----------------------------------------------
*    USBUART_PC_FORCE_J   | Force a J State onto the D+/D– lines
*    USBUART_PC_FORCE_K   | Force a K State onto the D+/D– lines
*    USBUART_PC_FORCE_SE0 | Force a Single Ended 0 onto the D+/D– lines
*    USBUART_PC_FORCE_NONE| Return bus to SIE control
*
*
*******************************************************************************/
void USBUART_PC_Force(uint8 bState) 
{
    ...
    */
    USBUART_PC_USBIO_CR0_REG = bState;
}


/*******************************************************************************
* Function Name: USBUART_PC_GetEPAckState
****************************************************************************//**
*
...
uint8 USBUART_PC_GetEPAckState(uint8 epNumber) 
{
    ...
    if ((epNumber > USBUART_PC_EP0) && (epNumber < USBUART_PC_MAX_EP))
    {
        cr = USBUART_PC_SIE_EP_BASE.sieEp[epNumber].epCr0 & USBUART_PC_MODE_ACKD;
    }
...
/*******************************************************************************
* Function Name: USBUART_PC_SetPowerStatus
****************************************************************************//**
*
...
*  --------------------------------------------|---------------------------
*  USBUART_PC_DEVICE_STATUS_BUS_POWERED  | Set the device to bus powered
*  USBUART_PC_DEVICE_STATUS_SELF_POWERED | Set the device to self powered
*
* \globalvars
*
*  \ref USBUART_PC_deviceStatus - set power status
*
* \reentrant
*  No.
*
...
{
    if (powerStatus != USBUART_PC_DEVICE_STATUS_BUS_POWERED)
    {
    ...
    /***************************************************************************
    * Function Name: USBUART_PC_VBusPresent
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return ((0u != (USBUART_PC_VBUS_STATUS_REG & USBUART_PC_VBUS_VALID)) ? (uint8) 1u : (uint8) 0u);
    }
#endif /* (USBUART_PC_VBUS_MONITORING_ENABLE) */


/*******************************************************************************
* Function Name: USBUART_PC_RWUEnabled
****************************************************************************//**
*
...
*
* \globalvars
*  USBUART_PC_deviceStatus - checked to determine remote status
*
*******************************************************************************/
uint8 USBUART_PC_RWUEnabled(void) 
{
    uint8 result = USBUART_PC_FALSE;

    if (0u != (USBUART_PC_deviceStatus & USBUART_PC_DEVICE_STATUS_REMOTE_WAKEUP))
    {
        result = USBUART_PC_TRUE;
    }
...
/*******************************************************************************
* Function Name: USBUART_PC_GetDeviceAddress
****************************************************************************//**
*
...
{
    return (uint8)(USBUART_PC_CR0_REG & USBUART_PC_CR0_DEVICE_ADDRESS_MASK);
}


/*******************************************************************************
* Function Name: USBUART_PC_EnableSofInt
****************************************************************************//**
*
...
void USBUART_PC_EnableSofInt(void) 
{
    ...
    USBUART_PC_INTR_SIE_MASK_REG |= (uint32) USBUART_PC_INTR_SIE_SOF_INTR;
#else
...
/*******************************************************************************
* Function Name: USBUART_PC_DisableSofInt
****************************************************************************//**
*
...
void USBUART_PC_DisableSofInt(void) 
{
    ...
    /***************************************************************************
    * Function Name: USBUART_PC_DetectPortType
    ************************************************************************//**
    *
    ...
    *   Return Value                      |Description
    *   ----------------------------------|-------------------------------------
    *   USBUART_PC_BCD_PORT_SDP     | Standard downstream port detected
    *   USBUART_PC_BCD_PORT_CDP     | Charging downstream port detected
    *   USBUART_PC_BCD_PORT_DCP     | Dedicated charging port detected
    *   USBUART_PC_BCD_PORT_UNKNOWN | Unable to detect charging port type (proprietary charger type)
    *   USBUART_PC_BCD_PORT_ERR     | Error condition in detection process
    *
    *
    * \sideeffects
    *
    ...
    ***************************************************************************/
    uint8 USBUART_PC_Bcd_DetectPortType(void)
    {
        ...
        uint8 result = USBUART_PC_BCD_PORT_UNKNOWN;
        ...
        */
        bkPwrCtrl = USBUART_PC_POWER_CTRL_REG;
        USBUART_PC_POWER_CTRL_REG = USBUART_PC_DEFAULT_POWER_CTRL_VBUS\
                            & (~USBUART_PC_POWER_CTRL_ENABLE_VBUS_PULLDOWN)\
                            & (~USBUART_PC_POWER_CTRL_ENABLE_DM_PULLDOWN);


        /* Enable PHY detector and single-ended and differential receivers.
         * Enable charger detection.  */
        USBUART_PC_POWER_CTRL_REG |= USBUART_PC_DEFAULT_POWER_CTRL_PHY\
                                         | USBUART_PC_POWER_CTRL_ENABLE_CHGDET;
        ...
        */
        #if defined (USBUART_PC_NO_DCD)
        CyDelay(USBUART_PC_BCD_TIMEOUT);
        #else
        /* DCD implementation:*/

        {
            uint16 timeout = USBUART_PC_BCD_TIMEOUT;
            uint8 connectionApproved = 0u;
            uint8 connected = 0u;

            /*   BCD spec 1.2: Turns on Idp_src and D- pull-down resistor */
            USBUART_PC_POWER_CTRL_REG |= USBUART_PC_POWER_CTRL_ENABLE_DM_PULLDOWN;
            USBUART_PC_CHGDET_CTRL_REG |= USBUART_PC_CHGDET_CTRL_DCD_SRC_EN;

            /* BCD spec 1.2: Waits for D+ to be low for a time of Tdcd_dbnc*/
            while ((0u != timeout) && (0u == connectionApproved))
            {
                if (0u == (USBUART_PC_USBIO_CR1_REG & USBUART_PC_USBIO_CR1_DP0))
                {
                    connected++;
                }
                else
                ...
                connectionApproved = (USBUART_PC_BCD_TDCD_DBNC < connected) ? 1u:0u;
                CyDelay(1u);
                timeout--;
            }
        ...
        #endif /*(USBUART_PC_NO_DCD)*/

        /* Primary detection: enable VDP_SRC on D+ and IDM_SINK on D-. */
        USBUART_PC_CHGDET_CTRL_REG = USBUART_PC_CHGDET_CTRL_PRIMARY;
        CyDelay(USBUART_PC_BCD_PRIMARY_WAIT);
        cr1RegVal = USBUART_PC_USBIO_CR1_REG;

        /* Check is it SDP or DCP/CDP, read comparator 2 output. */
        if (0u == (USBUART_PC_CHGDET_CTRL_REG & USBUART_PC_CHGDET_CTRL_COMP_OUT))
        {
            /* Check status of D- line. */
            if (0u == (cr1RegVal & USBUART_PC_USBIO_CR1_DM0))
            {
                result = USBUART_PC_BCD_PORT_SDP;
            }
            else
            {
                 ...
                 * detection.
                */
                result = USBUART_PC_BCD_PORT_ERR;
            }
        }
        else
        {
        ...
        {
            USBUART_PC_CHGDET_CTRL_REG = USBUART_PC_CHGDET_CTRL_DEFAULT \
                                                | USBUART_PC_CHGDET_CTRL_SECONDARY;
            CyDelay(USBUART_PC_BCD_SECONDARY_WAIT);
            cr1RegVal = USBUART_PC_USBIO_CR1_REG;
            ...
            {
                /* Check status of D+ line. */
                if (0u == (cr1RegVal & USBUART_PC_USBIO_CR1_DP0))
                {
                    result = USBUART_PC_BCD_PORT_CDP;
                }
                else
                {
                    /* ERROR: such combination is impossible. Abort charger
            ...
            else
            {
                ...
                if (0u == (cr1RegVal & USBUART_PC_USBIO_CR1_DP0))
                {
                    result = USBUART_PC_BCD_PORT_DCP;
                }
                else
                {
                    ...
                    */
                    result = USBUART_PC_BCD_PORT_UNKNOWN;
                }
            }
        }

        /* Restore CHGDET_CTRL. */
        USBUART_PC_CHGDET_CTRL_REG = 0u;

        /*Revert registers back*/
        USBUART_PC_POWER_CTRL_REG = bkPwrCtrl;
        USBUART_PC_USBIO_CR1_REG |= (uint32) USBUART_PC_USBIO_CR1_IOMODE;
        USBUART_PC_USB_CLK_EN_REG = ~USBUART_PC_USB_CLK_CSR_CLK_EN;
...
#if (USBUART_PC_LPM_ACTIVE)
    /***************************************************************************
    * Function Name: USBUART_PC_Lpm_GetBeslValue
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USBUART_PC_LPM_STAT_REG & USBUART_PC_LPM_STAT_LPM_BESL_MASK);
    }


    /***************************************************************************
    * Function Name: USBUART_PC_Lpm_RemoteWakeUpAllowed
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (USBUART_PC_LPM_STAT_REG & USBUART_PC_LPM_STAT_LPM_REMOTE_WAKE);
    }


    /***************************************************************************
    * Function Name: USBUART_PC_Lpm_SetResponse
    ************************************************************************//**
    *
    ...
    *       - USBUART_PC_LPM_REQ_ACK - next LPM request will be
    *                                           responded with ACK
    *       - USBUART_PC_LPM_REQ_NACK - next LPM request will be
    *                                           responded with NACK
    *       - USBUART_PC_LPM_REQ_NYET - next LPM request will be
    *                                           responded with NYET
    *
    ***************************************************************************/
    void USBUART_PC_Lpm_SetResponse(uint32 response)
    {
        uint32 lpmCtrl = USBUART_PC_LPM_CTRL_REG & (uint32) ~USBUART_PC_LPM_CTRL_ACK_NYET_MASK;

        USBUART_PC_LPM_CTRL_REG = lpmCtrl | ((uint32) response & USBUART_PC_LPM_CTRL_ACK_NYET_MASK);
    }


    /***************************************************************************
    * Function Name: USBUART_PC_Lpm_GetResponse
    ************************************************************************//**
    *
    ...
    *       - USBUART_PC_LPM_REQ_ACK - next LPM request will be responded
    *                                        with ACK
    *       - USBUART_PC_LPM_REQ_NACK - next LPM request will be responded
    *                                        with NACK
    *       - USBUART_PC_LPM_REQ_NYET - next LPM request will be responded
    *                                        with NYET
    *
    ***************************************************************************/
    uint32 USBUART_PC_Lpm_GetResponse(void)
    {
        ...
        return  ((uint32) USBUART_PC_LPM_CTRL_REG & (uint32)USBUART_PC_LPM_CTRL_ACK_NYET_MASK);
    }
