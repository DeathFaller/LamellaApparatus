...
/*******************************************************************************
* Function Name: Emul_EEPROM_Start
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Emul_EEPROM_Stop
********************************************************************************
*
* Summary:
*  Empty function. Included for consistency with other
...
/*******************************************************************************
* Function Name: Emul_EEPROM_Write
********************************************************************************
*
* Summary:
...
*******************************************************************************/
#if (CY_PSOC3)
    cystatus Emul_EEPROM_Write(const uint8 srcBuf[], const uint8 eepromPtr[], uint16 byteCount)
...
#endif /* (!CY_PSOC3) */
{
    ...
    #if (CY_PSOC3)
        eeOffset &= Emul_EEPROM_CODE_ADDR_MASK;
    #endif /* (CY_PSOC3) */

    /* Make sure, that eepromPtr[] points to ROM */
    #if (CY_PSOC3)
        if (((uint32)eepromPtr >= Emul_EEPROM_CODE_ADDR_OFFSET) &&
            (((uint32)eepromPtr + byteCount) <= Emul_EEPROM_CODE_ADDR_END))
    #else
        if (((uint32)eepromPtr + byteCount) <= Emul_EEPROM_FLASH_END_ADDR)
    #endif /* (CY_PSOC3) */
    {
        #if (!CY_PSOC4)
            (void)CySetTemp();
        ...
        rowId = (eeOffset / CY_FLASH_SIZEOF_ROW) % Emul_EEPROM_ROWS_IN_ARRAY;
        ...
        while ((srcIndex < byteCount) && (CYRET_SUCCESS == rc))
        {
            ...
            rowId++;
                ...
                if (rowId >= Emul_EEPROM_ROWS_IN_ARRAY)
                {
                    arrayId++;
                    rowId = 0u;
                }
