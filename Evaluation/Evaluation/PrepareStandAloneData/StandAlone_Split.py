...
def split(dataname):
    ...
    for elem in Lasers:
        ...
        smoothed = sp.savgol_filter(intensities, window_length=5, polyorder=1)
        for elem1 in elem:
            elem1.append(str(smoothed[elem.index(elem1)]))
    # speichern
    for elem in Lasers:
        f = open(
            dataname[0:-4]
...
if __name__ == "__main__":
    for RawDataFile in GFL.askFileList(
        ...
        t = threading.Thread(target=split, args=(RawDataFile, ))
        t.start()
